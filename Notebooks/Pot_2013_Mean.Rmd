---
title: "Potlatch/Lower Clearwater Mean 2013"
...
library(uuid)
  ...
  nameBasin <- "Lower Clearwater" #this is from the Projects XML list of approved names
  
  dataPath <- "D:/OneDrive/work/research/CHaMP/CHaMP_data/EP_temp/shapes/"
  mainPath <- "D:/OneDrive/work/research/CHaMP/CHaMP_data/Clearwater/"
...
```{r, message=FALSE}
  ...
  LST.in <- read.dbf(paste0(dataPath, "/", basin, "_", yrPath, "_LST.dbf"))
  PointID <- LST.in[,1]
  LST.in <- LST.in[,3:48]
...
out_Preds <- read.dbf(paste0(mainPath, longBasin, "/", "LST", yrPath, "_", basin, "_interp.dbf"))
  ...
  weights <- read.csv(paste0("../../", midBasin, "_area_wgts.csv"), stringsAsFactors=FALSE)
  ...
  for(i in rcas)	
    {
      pixels <- weights[weights$RCAID == i, "ID"]
...
```{r}
  ...
  Log.in <- read.csv(paste0("../../", basin, "_CBS_", yearPath, "_logger_data.csv"), stringsAsFactors = FALSE)
  colnames(Log.in)[3] <- "SiteID"
  ...
  for (i in SiteID) 
    { 
      ...
      eightdayMn <- rollapply(full.year$DailyMean, width = 8, FUN = mean, na.rm = T, fill = NA, align = c("left"))
      eightdayMn <- as.matrix(eightdayMn)
      full.year$Mn8D<- eightdayMn
      full.year$Mn8D[361] <- mean(full.year$DailyMean[361:365])
      
      eightdayMin <- rollapply(full.year$DailyMinimum, width = 8, FUN = mean, na.rm = T, fill = NA, align = c("left"))
      eightdayMin <- as.matrix(eightdayMin)
      full.year$Min8D<- eightdayMin
      full.year$Min8D[361] <- mean(full.year$DailyMinimum[361:365])
      
      eightdayMx <- rollapply(full.year$DailyMaximum, width = 8, FUN = mean, na.rm = T, fill = NA, align = c("left"))
      eightdayMx <- as.matrix(eightdayMx)
      full.year$Mx8D<- eightdayMx
  ...
  write.csv(Log.8Day.out, paste0("../", basin, "_", yearPath, "_8Day_logger.csv"), row.names = FALSE)
  
```
Model building
```{r}
  Log.in <- read.csv(paste0("../", basin, "_", yearPath, "_8Day_logger.csv"), stringsAsFactors = FALSE)

  RCA.in <- read.csv(paste0("../../../", midBasin, "_RCAID.csv"), stringsAsFactors = FALSE)
  rcas <- unique(RCA.in$RCAID)
  
  ID.in <- read.dbf(paste0("../../../", basin, "_sites_elev.dbf")) 
  colnames(ID.in) <- c("SiteID", "Elev", "RCAID")
  ...
  Dv1.in <- Dv.in[,c("Dv8D_z1", "Days")]
  
  Dv2.in <- Dv.in[,c("Dv8D_z2", "Days")]
    ...
    { 
      ...
      RCAID <- ID.in$RCAID[ID.in$SiteID == i]
        ...
        if (RCAID %in% rcas)
        {
          ...
          LST.Log.site <- merge(LST.Log.site, Dv1.in, by.x = "JulDay", by.y = "Days", all.x=TRUE, all.y+FALSE)
          LST.Log.site <- merge(LST.Log.site, Dv2.in, by.x = "JulDay", by.y = "Days", all.x=TRUE, all.y+FALSE)
  ...
  Dv1 <- unlist(LST.Log.site$Dv8D_z1)
  Dv2 <- unlist(LST.Log.site$Dv8D_z2)
  ind <- apply(LST.Log.out, 1, function(x) !any(is.na(x)))
  NoNA.xyz <- LST.Log.out[ind,]
  
  NoNA.xyz <- NoNA.xyz[,c(dataVar, "LST", "JulDay", "Elev", "Dv8D_z1", "Dv8D_z2", "SiteID")]
  colnames(NoNA.xyz) <- c("y", "x", "z", "e", "d1", "d2", "SiteName")
...
```{r}
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2) + d1 + d2, validation = "LOO")
  p2 <- R2(mod2)
...
```{r}    
  coeffs_out <- data.frame(Int=numeric(2), bLST=numeric(2), bLST2=numeric(2), bJul=numeric(2), bElev=numeric(2), bD1=numeric(2), bD2=numeric(2))
...
```{r}
    ...
    coeffs_out <- data.frame(Int=numeric(2), bLST=numeric(2), bLST2=numeric(2), bJul=numeric(2), bE=numeric(2), bD=numeric(2))
  metrics_out <- data.frame(r2=numeric(2), RMSE=numeric(2), p2=numeric(2), RMSEP=numeric(2), N_Sites=numeric(2), N=numeric(2))
  rownames(metrics_out) <- c("Spring", "Fall")
  rownames(coeffs_out) <- c("Spring", "Fall")
  rownames(metrics_out) <- c("Spring", "Fall")
...
```{r}
  data.sp <- data.sp[-c(962, 959, 1344, 1286, 961, 1121), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r, message=FALSE}
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2) + z + d1 + d2, validation = "LOO")
  p2 <- R2(mod2)
  detach("package:pls", unload=TRUE)
```
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(6), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
   ...
   for (i in 1:length(rcas))  
      {
        ...
        j <- 1
        for (l in 1:midrow)
        {x[l] <- x[l] * coeffs.in$bLST[1] + j * coeffs.in$bJul[1] +  x[47] * coeffs.in$bE[1] + Dv1[l] * coeffs.in$bD1[1] + Dv2[l] * coeffs.in$bD2[1]+ x[l]^2 * coeffs.in$bLST2[1] + coeffs.in$Int[1]
        j <- j + 8}
        k <- day
        for (l in maxrow:46)     
        {x[l] <- x[l] * coeffs.in$bLST[2] + k * coeffs.in$bJul[2] +  x[47] * coeffs.in$bE[2] + Dv1[l] * coeffs.in$bD1[2] + Dv2[l] * coeffs.in$bD2[2] +  x[l]^2 * coeffs.in$bLST2[2] + coeffs.in$Int[2]
        k <- k + 8}
        if (maxrow > 3)
        {
          x[(midrow)] <- NA
...
```{r}  
  ...
  points(namesnum, LogPred.out[500, 1:46], pch=16, col="red")
...
write.dbf(Error.pts.out, file = paste0("Error", yearPath, "_", midBasin, "_8D_", var, ".dbf")) 
write.csv(Error.pts.out, file = paste0("Error", yearPath, "_", midBasin, "_8D_", var, ".csv"), row.names = FALSE)
```
...
```{r}
  ...
  netname <- paste0(longBasin, "_RCA_net")
  ptsname <- paste0(midBasin, "_sites")
  
  error_pts <- readOGR(dsn=gisPath, ptsname)
  
  colnames(error_pts@data)[2] <- "SiteID"
  
  network <- readOGR(dsn=gisPath, layer = netname)
  net = network[network$RCAID %in% rcas,]
  network = net
  ...
  for (i in 3:48)
    {
      ...
      legend("topright", fill = attr(fix3.colors, "palette"), title="°C", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.05,0.1), text.col="white");
      legend(x=grconvertX(c(0.85,0.95), from='npc'), 
        y=grconvertY(c(0.3, 0.45), from='npc'), pch=16, col="gray40", pt.cex=c(0.5, 0.75, 1.0, 1.25), title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", cex=.5, text.col="white");
      
      
      title(paste0(longBasin, " 8-day ", var, " ", yearPath, " (°C)"), line=-0.6, adj=0.1, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5)
      ...
      tmp2 <- subplot(
        ...
        x=grconvertX(c(0.6,0.95), from='npc'), 
        y=grconvertY(c(0.08, 0.20), from='npc'),
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Pot_2013_8D_Mn.mpeg')
```
...
```{r}
  ...
  filename <- paste0(modelPath, "/graphics/Stills/", midBasin, "_", yearPath, "_8D_", var, ".png", sep="")
    png(filename=filename, res = 300, width = 1500, height = 1500, units = "px", bg="white")
        ...
        legend(x=grconvertX(c(0.85,0.95), from='npc'), 
        y=grconvertY(c(0.3, 0.45), from='npc'), pch=16, col="gray40", pt.cex=c(0.5, 0.75, 1.0, 1.25), title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", cex=.5, inset=c(0,0), text.col="black");
