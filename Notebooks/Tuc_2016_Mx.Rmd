...
Plot to look
```{r}
{plot(1:46, CLST.out[562,1:46])
  points(1:46, CLST.out[800, 1:46], pch=16, col="blue")
  points(1:46, CLST.out[1000, 1:46], pch=16, col="green")
  points(1:46, CLST.out[1, 1:46], pch=16, col="red")
  points(1:46, CLST.out[1149, 1:46], pch=16, col="lightblue")}
```
Write it out
```{r}
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(840,874,6,39), ])
```
...
```{r}
  data.sp <- data.sp[-c(840,874,6,39), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.fall[-c(25), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
  ...
  for (i in 3:48)
    {
      ...
      namey <- gsub(paste0(varName, "_"), "", colnames(network@data)[i])
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Tuc_2016_8D_Mx.mpeg')
```
