...
Model building
```{r}
  Log.in <- read.csv(paste0(basin, "_", yearPath, "_logger", ".csv"), stringsAsFactors = FALSE)
  colnames(Log.in)[5] <- "JulDay"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + e, data=NoNA.xyz[-c(54), ])
```
...
```{r}  
  ...
  points(namesnum, LogPred.out[280, 1:46], pch=16, col="red")
  points(namesnum, LogPred.out[100, 1:46], pch=16, col="lightblue")}
...
```{r}
  ...
  #   }
  # 
  #   net = network[network$RCAID %in% rcaList,]
  net <- network
  names.out <- colnames(net@data[3:48])
  namesnum <- as.numeric(gsub(paste0("T", var, yrPath, "_"), "", colnames(net@data[3:48])))
  means <- colMeans(net@data[3:48], na.rm = TRUE)
  SDs <- colStdevs(net@data[3:48], na.rm = TRUE)
  yplus <- means + SDs
  yminus <- means - SDs
  ...
  for (i in 3:48)
    {
      ...
      plot(net, col=fix3.colors, bg="black", fg="white")
      points(error_pts, pch=16, col="gray40", cex=cexEr)
      
       legend(x=grconvertX(c(0.85, 0.95), from='npc'), 
             y=grconvertY(c(0.55, .3), from='npc'), fill = attr(fix3.colors, "palette"), title=paste0("8-day ", longVar, " (°C)"),legend = c("0-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20", "20-22", "22+"), bty = "n", cex=.5, inset=c(0,0), text.col="white");
      ...
      tmp2 <- subplot(
        ...
        x=grconvertX(c(0.09,0.44), from='npc'), 
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png YF_2013_8D_Mx.mpeg')
```
...
```{r}
  ...
  filename <- paste0(modelPath, "/graphics/Stills/", basin, "_", yearPath, "_8D_", var, ".png", sep="")
    png(filename=filename, res = 300, width = 1500, height = 1500, units = "px", bg="white")
        ...
        legend(x=grconvertX(c(0.85, 0.95), from='npc'), 
             y=grconvertY(c(0.55, .3), from='npc'), fill = attr(fix3.colors, "palette"), title=paste0("8-day ", var, " (°C)"),legend = c("0-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20", "20-22", "22+"), bty = "n", cex=.5, inset=c(0,0), text.col="black");
