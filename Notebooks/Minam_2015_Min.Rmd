...
```{r, message=FALSE}
  ...
  LST.in <- read.dbf(paste0(gisPath, "/", longBasin, "/", midBasin, "_", yrPath, "_LST.dbf"))
  PointID <- LST.in[,1]
  LST.in <- LST.in[,2:47]
...
```{r}
  ...
  #edit appropriate columns as necessary to get correct variable
  NoNA.xyz <- NoNA.xyz[,c(8, 2, 1, 3, 4)]
...
```{r}
  ...
  errorName <- sprintf("%s_%03d", "JulDay", newnamesnum)
