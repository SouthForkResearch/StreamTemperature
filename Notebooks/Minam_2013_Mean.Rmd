...
library(uuid)
  ...
  nameBasin <- "Wallowa" #this is from the Projects XML list of approved names
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(22,96), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
```{r, message=FALSE}
  ...
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.7, 0.85), from='npc'), 
             y=grconvertY(c(0.72, 0.92), from='npc'), fill = attr(fix3.colors, "palette"), legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Minam_2013_8D_Mn.mpeg')
```
...
```{r}
  ...
  filename <- paste0(modelPath, "/graphics/Stills/", basin, "_", yearPath, "_8D_", var, ".png", sep="")
    ...
    legend(x=grconvertX(c(0.7, 0.85), from='npc'), 
             y=grconvertY(c(0.72, 0.92), from='npc'), fill = attr(fix3.colors, "palette"), legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="black");
      legend(x=grconvertX(c(0.05, 0.25), from='npc'), 
