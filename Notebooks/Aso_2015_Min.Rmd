...
library(uuid)
  ...
  var <- "Min"
  longVar <- "Min"
  dataVar <- "Min8Day" #name of the variable in the logger data
  capVar <- "MIN"
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(167),])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Asotin_2015_8D_Min.mpeg')
```
