...
```{r}
  ...
  newnamesnum <- as.numeric(colnames(LST.in)[2:47]);
...
```{r}
  data.sp <- data.sp[-c(%holder for IDs%), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(147), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
```{r}
  ...
  points(namesnum, LogPred.out[250, 1:46], pch=16, col="green")
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Tucannon_2012_8D_Mn.mpeg')
```
