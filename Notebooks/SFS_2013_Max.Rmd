---
title: "South Fork Salmon 2013 Max"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(400), ])
```
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(8,11), ])
```
check for outliers
```{r}
  outlierTest(mod)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png SFS_2013_8D_Max.mpeg')
```
