...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.sp[-c(965,993,1032,875,795,915), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}
  data.sp <- data.sp[-c(965,993,1032,875,795,915), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  ...
  network@data <- network@data[,c("rca_id", "REACHCODE")]
  ...
  for (i in 3:48)
    {
      ...
      points(error_pts, pch=16, col="gray40", cex=cexEr)
       ...
       legend(x=grconvertX(c(0.85,0.9), from='npc'), 
        y=grconvertY(c(0.3, 0.55), from='npc'), fill = c("#0024E3", "#0087CD", "#16F45A", "#3FFA36", "#BDFF0C", "#FFDD00", "#FF9000", "#FF4400", "#F70000", "#AA0000"), legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png YF_2016_8D_Min.mpeg')
```
