...
library(classInt)
  ...
  var <- "Mn"
  longVar <- "Mean"
  dataVar <- "Mn8DayAvg" #name of the variable in the logger data
  capVar <- "MEAN"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(342,344,366,367), ])
```
...
```{r}
  data.sp <- data.sp[-c(342,344,366,367), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}  
  {plot(namesnum, LogPred.out[252,1:46])
  points(namesnum, LogPred.out[800, 1:46], pch=16, col="blue")
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Wen_2013_8D_Max.mpeg')
```
