...
library(classInt)
 ...
 basin <- "Panth"
  ...
  nameBasin <- "Middle Salmon-Panther"
  ...
  modelPath <- paste0(mainPath, "USal/", longBasin, "/", yearPath, "/", longVar, "/")
...
```{r}
  mod2 <- lm(y ~ x + I(x^2) + e, data=NoNA.xyz[-c(237,248,259,281,286,308), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod2)
...
```{r}
  NoNA.xyz2 <- NoNA.xyz[-c(237,248,259,281,286,308), ]
  y <- NoNA.xyz2$y
  x <- NoNA.xyz2$x
  z <- NoNA.xyz2$z
  e <- NoNA.xyz2$e
...
rerun w/o outliers (if necessary)
```{r}
  mod2 <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(6,9,17,20), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod2)
