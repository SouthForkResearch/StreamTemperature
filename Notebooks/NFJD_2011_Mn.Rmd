---
title: "North Fork John Day 2011 Mean"
...
```{r}
  {plot(newnamesnum, LogPred.out[400,1:46])
  ...
  points(newnamesnum, LogPred.out[300, 1:46], pch=16, col="lightblue")}
...
```{r}
  ...
  ptsname <- paste0(basin, "_sites_RCAID")
  
  error_pts <- readOGR(dsn=gisPath, ptsname)
  error_pts@data <- error_pts@data[,c(2,4)]
  ...
  for (i in 3:48)
    {
      ...
      legend( x=grconvertX(c(0.90,0.99), from='npc'), 
        y=grconvertY(c(0.7, 0.95), from='npc'),
        fill = attr(fix3.colors, "palette"), title="°C", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.05,0.1), text.col="white");
      legend("bottomleft", pch=16, col="gray40", pt.cex=c(0.5, 0.75, 1.0, 1.25), title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", cex=.5, inset=c(0.0,0), text.col="white");
      
      
      title(paste0(titleBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.6, adj=0, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png NFJD_2011_8D_Mn.mpeg')
```
