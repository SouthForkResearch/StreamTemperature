...
```{r, message=FALSE}
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2) + z + d + d*z, validation = "LOO")
  p2 <- R2(mod2)
  detach("package:pls", unload=TRUE)
```
...
```{r}
  mod2 <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(6), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod2)
...
```{r}
  ...
  network <- merge(network, Pred.in, by = "RCAID")                    
