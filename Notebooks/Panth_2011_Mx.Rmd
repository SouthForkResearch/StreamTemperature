---
title: "Mid Salmon/Panther Creek Max 2011"
...
```{r}
  mod2 <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(1,16,23,88,110), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod2)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(1,16,23,88,110), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Panther_2011_8D_Mx.mpeg')
```
