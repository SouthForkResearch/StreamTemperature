...
library(uuid)
  ...
  nameBasin <- "Tucannon" #this is from the Projects XML list of approved names
  ...
  var <- "Mx"
  longVar <- "Max"
  dataVar <- "X8DayAvgOfMax" #name of the variable in the logger data
  capVar <- "MAX"
  
  modelPath <- paste0(mainPath, "EP_Temp/", basin, "/", yearPath, "/", longVar, "/")
...
```{r}
  ...
  NoNA.xyz <- read.csv(paste0(basin, "_", yearPath, "_8Day", "_model_data.csv"), stringsAsFactors = FALSE)
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(512), ])
```
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(##), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
```{r}  
  {plot(namesnum, LogPred.out[535,1:46])
  points(namesnum, LogPred.out[100, 1:46], pch=16, col="blue")
  points(namesnum, LogPred.out[123, 1:46], pch=16, col="green")
  points(namesnum, LogPred.out[62, 1:46], pch=16, col="red")
...
```{r}
  ...
  Log.in <- read.csv(paste0(mainPath, "/", longBasin, "/", basin, "_logger_8D_", yearPath, ".csv"), stringsAsFactors = FALSE)
...
```{r, message=FALSE}
  ...
  ptsname <- paste0(basin, "_CHaMP_sites_rca")
  
  error_pts <- readOGR(dsn=gisPath, ptsname)
  error_pts@data <- error_pts@data[,c(3,7)]
  ...
  for (i in 3:48)
    {
     ...
     error <- error_pts@data[,i]
      ...
      cexEr <-ifelse(error == 0, 0,
                     ifelse(error>0 & error <1, 0.5,
                            ifelse(error>1 & error<2, 0.75,
                                   ifelse(error>2 & error<3, 1.0,
                                          ifelse(error>3, 1.25,
                                                 ifelse(error== NA, 0, NA))))))
      
      plot(network, col=fix3.colors, bg="black", fg="white")
      points(error_pts, pch=16, col="gray40", cex=cexEr)
      
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
             y=grconvertY(c(0.35, 0.55), from='npc'), fill = attr(fix3.colors, "palette"), title="8-day Mn (°C)", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      ...
      title(paste0(longBasin, " 8-day ", var, " ", yearPath, " (°C)"), line=-0.9, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Tucannon_2011_8D_Mx.mpeg')
```
...
```{r}
  ...
  namey <- gsub(paste0("T", var, "_", yrPath, "_"), "", colnames(network@data)[i])
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
             y=grconvertY(c(0.35, 0.55), from='npc'), fill = attr(fix3.colors, "palette"), title="8-day Mn (°C)", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="black");
