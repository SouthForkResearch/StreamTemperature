...
```{r}
  ...
  ptsname <- paste0("ent1km_pts")
  pts <- readOGR(dsn=dsnPath, layer=ptsname)
  
  outPath <- paste0(gisPath, yearPath, "/")
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(667,680,627,32,646,56), ])
```
...
```{r}
  data.sp <- data.sp[-c(667,680,627,32,646,56), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(17,30), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}  
  ...
  points(namesnum, LogPred.out[350, 1:46], pch=16, col="blue")
  points(namesnum, LogPred.out[100, 1:46], pch=16, col="green")
  points(namesnum, LogPred.out[1, 1:46], pch=16, col="red")
  points(namesnum, LogPred.out[121, 1:46], pch=16, col="lightblue")}
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Ent_2016_8D_Mn.mpeg')
```
...
```{r}
  ...
  filename <- paste0(gisPath, "/graphics/", basin, "_", yearPath, "_curr_", var, ".png", sep="")
