...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(226,235), ])
```
...
```{r, message=FALSE}
  ...
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
      ...
      title(paste0(longBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=.5, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Met_2013_8D_Mx.mpeg')
```
