...
Plot to look
```{r}
{plot(1:46, rca_zonal[rca_zonal$RCAID==667,1:46])
  points(1:46, rca_zonal[rca_zonal$RCAID==3, 1:46], pch=16, col="blue")
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + e + d, data=data.sp[-c(1490,1226,1278,973), ])
```
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(1226,1278,1329,1174,881,1227,1224,1228,1303), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}
  data.sp <- data.sp[-c(1226,1278,1329,1174,881,1227,1224,1228,1303), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Lochsa_2013_8D_Mx.mpeg')
```
