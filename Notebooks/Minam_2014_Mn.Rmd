...
```{r}
  ...
  var <- "Mn"
  longVar <- "Mean"
  dataVar <- "X8DayMn" #name of the variable in the logger data
  capVar <- "MEAN"
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(54), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Minam_2014_8D_Mn.mpeg')
```
