...
Edited 1 Sept 2017 to add JulDay * Discharge interaction term
...
```{r}
  ...
  writeOGR(pts.out, dsn=dsnPath, layer=pts.out.name, driver="ESRI Shapefile")
```
...
```{r}
  ...
  weights <- read.csv(paste0(mainPath, midBasin, "/", basin, "_rca_area_wgts.csv"), stringsAsFactors=FALSE)
  ...
  for(i in rcas)	
    {
      ...
      for (j in 1:46)
      {
        ...
        zonal_mn <- round(weighted.mean(daily, wgts, na.rm = TRUE),2)
...
```{r}
  ...
  write.dbf(rca_zonal, file = paste0(mainPath, midBasin, "/", "LST", yrPath, "_", basin, "_RCA.dbf"))
...
```{r}
  ...
  Log.in <- read.csv(paste0("../../", basin, "_logger_8Day_", yearPath, ".csv"), stringsAsFactors = FALSE)
...
```{r}
  ...
  coeffs_out <- data.frame(Int=numeric(2), bLST=numeric(2), bLST2=numeric(2), bJul=numeric(2), bE=numeric(2), bD=numeric(2), bDJul=numeric(2))
  metrics_out <- data.frame(r2=numeric(2), RMSE=numeric(2), p2=numeric(2), RMSEP=numeric(2), N_Sites=numeric(2), N=numeric(2))
  rownames(metrics_out) <- c("Spring", "Fall")
  rownames(coeffs_out) <- c("Spring", "Fall")
...
rerun w/o outliers
```{r}
  mod2 <- lm(y ~ x + I(x^2) + z + e + d*z, data=data.sp[-c(584), ])
```
...
```{r}
  mod2 <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.fall[-c(##), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod2)
...
```{r}
  {plot(newnamesnum, LogPred.out[3000,1:46])
  points(newnamesnum, LogPred.out[500, 1:46], pch=16, col="blue")
  points(newnamesnum, LogPred.out[1000, 1:46], pch=16, col="green")
  points(newnamesnum, LogPred.out[10, 1:46], pch=16, col="red")
  points(newnamesnum, LogPred.out[4000, 1:46], pch=16, col="lightblue")}
...
```{r}
  ...
  fix4 <- classIntervals(means, n = 11, style = "fixed",fixedBreaks=c(-1,4,6,8,10,12,14,16,18,20))
  ...
  for (i in 3:48)
    {
      ...
      title(paste0(titleBasin, " 8-day ", var, " ", yearPath, " (°C)"), line=-0.6, adj=1, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png JD_2015_8D_Min.mpeg')
```
...
```{r}
  ...
  filename <- paste0(modelPath, "/graphics/Stills/", basin, "_", yearPath, "_8D_", var, ".png", sep="")
    png(filename=filename, res = 300, width = 1600, height = 1500, units = "px", bg="white")
        ...
        mtext(paste0(titleBasin, " ",  yearPath, " 8-day ", var, " (°C)"),side =3, outer=TRUE, line=-3, col="black", cex=.7)
