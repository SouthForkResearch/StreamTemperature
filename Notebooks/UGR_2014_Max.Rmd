---
title: "Upper Grande Ronde 2014 Max"
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.fall[-c(901,857,905,860,762), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(901,857,905,860,762), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png UGR_2014_8D_Max.mpeg')
```
