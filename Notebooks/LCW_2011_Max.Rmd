---
title: "Lower Clearwater Max 2011"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d1 + d2, data=data.sp[-c(1646,1722,1298), ])
```
...
```{r}
  data.sp <- data.sp[-c(1646,1722,1298), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png LCW_2011_8D_Mx.mpeg')
```
