---
title: "South Fork Salmon 2014 Mean"
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(426), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
```{r}
  ...
  colnames(Pred.in)<-gsub("_14_", "", colnames(Pred.in))
...
```{r, message=FALSE}
  ...
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
      ...
      title(paste0(nameBasin, " 8-day ", var, " ", yearPath, " (°C)"), line=0.7, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png SFS_2014_8D_Mn.mpeg')
```
