---
title: "Upper Grande Ronde 2013 Max"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(1332,1324,1116), ])
```
...
```{r}
  data.sp <- data.sp[-c(1332,1324,1116), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r, message=FALSE}
  ...
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.8, 0.9), from='npc'), 
             y=grconvertY(c(0.72, 0.92), from='npc'), fill =  c("#0024E3","#0087CD","#16F45A","#73FF1A","#FFE200","#FFB700","#FF6A00","#FF1D00","#D00000","#AA0000"), title="°C", legend = c("0-4","4-8","8-10","10-12","12-14","14-16","16-18","18-20","20-22","22+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      legend(x=grconvertX(c(0.8, 0.9), from='npc'), 
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png UGR_2013_8D_Max.mpeg')
```
