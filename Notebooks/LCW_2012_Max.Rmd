---
title: "Lower Clearwater Max 2012"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d1 + d2, data=data.sp[-c(1859,1102, 1106, 1858), ])
```
...
```{r}
  data.sp <- data.sp[-c(1859,1102, 1106, 1858), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d1 + d2, data=data.fall[-c(2,77,1,71,362), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(2,77,1,71,362), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png LCW_2012_8D_Mx.mpeg')
```
