...
```{r}
  ...
  nameBasin <- "Upper Salmon" #this is from the Projects XML list of approved names
  
  dataPath <- "D:/OneDrive/work/GIS/8_day_1km_LST/LST_s12_"
  mainPath <- "D:/OneDrive/work/research/CHaMP/CHaMP_data/USal/"
...
{plot(1:46, CLST.out[1000,1:46])
  ...
  points(1:46, CLST.out[3545, 1:46], pch=16, col="green")
...
Plot to look
```{r}
{plot(1:46, rca_zonal[rca_zonal$RCAID==560,1:46])
  points(1:46, rca_zonal[rca_zonal$RCAID==1431, 1:46], pch=16, col="blue")
  points(1:46, rca_zonal[rca_zonal$RCAID==636, 1:46], pch=16, col="green")
  points(1:46, rca_zonal[rca_zonal$RCAID==2147, 1:46], pch=16, col="red")
...
```{r}
  ...
  Log.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/", basin, "/", basin, "/", basin, "_CBS_NorWeST_", yearPath, "_logger_data.csv"), stringsAsFactors = FALSE)
  ...
  colnames(Log.8Day.out)[1] <- "JulDay"
  
  write.csv(Log.8Day.out, paste0("../../", basin, "_logger_8D_", yearPath, ".csv"), row.names = FALSE)
...
```{r, message=FALSE}
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2) + z + d, validation = "LOO")
  p2 <- R2(mod2)
  detach("package:pls", unload=TRUE)
```
...
```{r}  
  ...
  points(namesnum, LogPred.out[1234, 1:46], pch=16, col="blue")
  points(namesnum, LogPred.out[3454, 1:46], pch=16, col="green")
  points(namesnum, LogPred.out[362, 1:46], pch=16, col="red")
...
```{r}
  ...
  colnames(error_pts@data)[2] <- "RCAID"
...
```{r, message=FALSE}
  ...
  graphicsPath <- paste0(mainPath, longBasin, "/", yearPath, "/", longVar, "/graphics/")
```
```{r}
  ...
  for (i in 3:48)
    {
      ...
      filename <- paste0(graphicsPath, namer[i-2], ".png", sep="")
    ...
    legend(x=grconvertX(c(0.8, 0.9), from='npc'), 
             y=grconvertY(c(0.72, 0.92), from='npc'), fill = attr(fix3.colors, "palette"), title="°C", legend = c("0-4","4-8","8-10","10-12","12-14","14-16","16-18","18-20","20-22","22+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      legend(x=grconvertX(c(0.8, 0.9), from='npc'), 
      ...
      title(paste0(longBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.1, adj=0.7, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
...
```{r}
setwd(paste0(graphicsPath))
  
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png USal_2012_8D_Min.mpeg')
```
...
cat("", file=paste("project.rs.xml",sep=""))
...
",sep=""), file=paste("project.rs.xml",sep=""), append=T)
