...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.sp[-c(720,628), ])
```
...
```
If the spatial data needs to be reloaded for graphics
```{r}
  outnetname <- paste0(basin, "_", yearPath, "_8D_", var)
  outptsname <- paste0(basin, "_Error_", yearPath, "_8D_", var)
  error_pts <- readOGR(dsn=".", outptsname)
  network <- readOGR(dsn=".", outnetname)
```
  ...
  for (i in 3:48)
    {
      ...
      title(paste0(titleBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.6, adj=1, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png JD_2016_8D_Mn.mpeg')
```
