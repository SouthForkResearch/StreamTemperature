...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + e + d, data=data.sp[-c(1527,1434,1419), ])
```
...
```{r}
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2) + d, validation = "LOO")
  p2 <- R2(mod2)
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(1419,1329,1381,1226,1434), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}
  data.sp <- data.sp[-c(1419,1329,1381,1226,1434), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Lochsa_2013_8D_Mn.mpeg')
```
