...
```{r, message=FALSE}
  ...
  LST.in <- read.dbf(paste0(mainPath, "/", basin, "/", basin, "_", yrPath, "_LST.dbf"))
  PointID <- LST.in[,1]
  LST.in <- LST.in[,2:47]
...
```{r}
  ...
  weights <- read.dbf(paste0("../../", basin, "_rca_area_wgt.dbf"))
...
Plot to look
```{r}
{plot(1:46, rca_zonal[rca_zonal$RCAID==1151,1:46])
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(47,113,73,102,1,91,45,27), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(47,113,73,102,1,91,45,27), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
...
```{r}  
  {plot(namesnum, LogPred.out[1234,1:46])
...
setwd(paste0(graphicsPath))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png USal_2011_8D_Mn.mpeg')
```
