---
title: "Upper Grande Ronde 2015 Min"
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(634), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
```{r, message=FALSE}
  ...
  for (i in 3:48)
   ...
   tmp2 <- subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,22), cex.main=.8, main="Basin mean (+/-SD)", line=1, adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png UGR_2015_8D_Min.mpeg')
```
