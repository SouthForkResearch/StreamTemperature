...
```{r}
 basin <- "Pahs"
  ...
  nameBasin <- "Pahsimeroi" #this is from the Projects XML list of approved names
...
```{r, message=FALSE}
  ...
  # if first column is empty for the entire grid (so a mean isn't possible)
  # LST.in[,1] <- LST.in[,2]
...
{plot(1:46, CLST.out[562,1:46])
  points(1:46, CLST.out[114, 1:46], pch=16, col="blue")
...
out_Preds <- read.dbf(paste0(mainPath, longBasin, "/", "LST", yrPath, "_", basin, "_interp.dbf"))
  ...
  weights <- read.csv(paste0(mainPath, longBasin, "/", basin, "_area_wgts.csv"), stringsAsFactors=FALSE)
...
```{r}
  ...
  write.csv(Log.8Day.out, paste0("../../", basin, "_CBS_NorWeST_", yearPath,  "_logger_data.csv"), row.names = FALSE)
...
rerun w/o outliers
```{r}
  mod2 <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(##), ])
```
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(5,13,29,21), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}
  qqPlot(mod2, main="QQ Plot Fall Leg")
```
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(5,13,29,21), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
  ...
  for (i in 3:48)
    {
      ...
      legend("topleft", fill = attr(fix4.colors, "palette"),  title="°C", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.05,0.1), text.col="white");
      legend(x=grconvertX(c(0.8,0.95), from='npc'), 
        y=grconvertY(c(0.1, 0.25), from='npc'),
        pch=16, col="gray40", pt.cex=c(0.5, 0.75, 1.0, 1.25), title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", cex=.5, inset=c(0.05,0.5), text.col="white");
      
      
      title(paste0(longBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.6, adj=0, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Pahs_2011_8D_Max.mpeg')
```
