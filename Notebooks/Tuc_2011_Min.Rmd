...
```{r}
  ...
  write.table (x=pred.out,append=F,row.names=F,file=paste0("jk_pred_v_y_", var, "_", basin, "_", yearPath, "_full_year.csv"),sep = ",", col.names=T)  
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(90), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r, message=FALSE}
  pred.y <- read.csv(paste0("jk_pred_v_y_", var, "_", basin, "_", yearPath, "_full_year.csv"), stringsAsFactors = FALSE, header = TRUE)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Tuc_2011_8D_Min.mpeg')
```
