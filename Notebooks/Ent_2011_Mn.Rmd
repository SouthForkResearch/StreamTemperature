...
library(uuid)
  ...
  nameBasin <- "Entiat" #this is from the Projects XML list of approved names
...
```{r}
  ...
  NoNA.xyz <- NoNA.xyz[,c("Avg8Day", "LST", "JulDay", "Elev_M", "SiteName")]
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(20,50,52,236,652,658,692,686), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(20,50,52,236,652,658,692,686), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
