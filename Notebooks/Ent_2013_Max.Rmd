...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(471), ])
```
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.sp[-c(50,52,20,22,51), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(50,52,20,22,51), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Ent_2013_8D_Mx.mpeg')
```
