...
```{r}
  ...
  for (i in SiteID) 
    { 
      ...
      full.year <- merge(dates, Log.site, by = "Days", all.x = TRUE)
...
```
Merge the CBS/NorWeST data with the CHaMP data by hand
...
Model building
```{r}
  Log.in <- read.csv(paste0("../../", basin, "_8Day_", var, "_logger_", yearPath, ".csv"), stringsAsFactors = FALSE)
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + d + d*z, data=data.fall[-c(##), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Lemhi_2011_8D_Mn.mpeg')
```
