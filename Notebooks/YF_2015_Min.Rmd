...
```{r}
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2) + e + d + d*z, validation = "LOO")
  p2 <- R2(mod2)
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(722,718), ])
```
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png YF_2015_8D_Min.mpeg')
```
