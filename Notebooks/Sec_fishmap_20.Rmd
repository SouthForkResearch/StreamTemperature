...
```{r}
  ...
  #namesnum <- as.numeric(gsub(paste0("Tmx_", yrPath, "_"), "", colnames(network@data[3:48])))
```
Set dispPerc once at the begining
```{r}
  dispPerc <- numeric(80) #96 for 2011-2016
...
```{r}
  for (i in 65:80) #2011 1:16, 2012 17:32, 2013 33:48, 2014 49:64, 2015 65:80, 2016 81:96  June 10-Oct 15 #21:36 
    {
      
      j <- i-44  # 2011 i+20, 2012 i+4, 2013 i-12, 2014 i-28, 2015 i-44, 2016 i-60
    ...
    legend(x=grconvertX(c(0.80, 0.99), from='npc'),
             y=grconvertY(c(0.65, 0.85), from='npc'), fill = c("#0000CD", "#C1CDCD"), title="8-day mean max", legend = c("< 20 °C", "> 20 °C"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      ...
      #change xlim
       tmp2 <- subplot(
        plot(sequ[1:i], dispPerc[1:i], type="l", col="lightblue", bty="n", ylim=c(0,100), xlim=c(1,80), xlab="     2011      2012      2013      2014      2015", ylab="", cex.lab=0.9, cex.main=.95, main="Percent usable stream length", adj=0, line=.5, col.lab="white", col.main = "white", cex.axis=0.6, col.axis="white", bg="black", yaxp=c(0,100,2), xaxp=c(2011,2015,2)),
...
setwd(paste0(otherOutPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 15 -morph 3 *.png Sec_Chinook_20.mpeg')
```
...
```{r}
      i <- length(dispPerc)
      j <- m-12 # pull this from the appropriate year 2011 i+20, 2012 i+4, 2013 i-12, 2014 i-28, 2015 i-44, 2016 i-60
    ...
    legend(x=grconvertX(c(0.80, 0.99), from='npc'),
             y=grconvertY(c(0.65, 0.85), from='npc'), fill = c("#0000CD", "#C1CDCD"), title="8-day mean max", legend = c("< 20 °C", "> 20 °C"), bty = "n", cex=.5, inset=c(.1,0), text.col="black");

      par(oma=c(0,0,0,0))
      
      title(paste0("Available Chinook holding and spawning habitat in the ", nameBasin,", 10 June - 15 Oct, 2011-2015"), line=-0.1, adj=.80, col.main="black", col.sub="black", outer=FALSE, cex.main=0.6, cex.sub=0.5)
      ...
      mtext(paste0(yearPath),side =1, outer=TRUE, line=-3, col="black", cex=.75)
       ...
       tmp2 <- subplot(
        plot(sequ[1:i], dispPerc[1:i], type="l", col="blue", bty="n", ylim=c(0,100), xlim=c(1,96), xlab="2011     2012     2013     2014     2015", ylab="", cex.lab=.9, cex.main=.95, main="Percent usable stream length", adj=0, line=.5, col.lab="black", col.main = "black", cex.axis=0.6, col.axis="black", bg="white", yaxp=c(0,100,2), xaxp=c(2011,2016,2)),
      ...
      par(tmp2)
      points(sequ[m], dispPerc[m], pch=16, col="red", cex=0.6)
      par(op)
...
```{r}
  ...
  plot(sequ[1:j], dispPerc[1:j], type="l", col="blue", bty="n", ylim=c(0,110), xlim=c(1,96),
       ...
       main=paste0("Chinook holding and spawning habitat in the ", nameBasin, " under ", temp, " °C, 2011-2015 \n \n Percent usable stream length"), adj=0, line=0.1, col.lab="black", col.main = "black", 
