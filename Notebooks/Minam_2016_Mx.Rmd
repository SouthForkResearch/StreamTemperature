...
```
This part reads in the 1km points and writes them back out to use for the current year
```{r}
  
  dsnPath <- (paste0(gisPath, "/", longBasin))
  
  ptsname <- paste0("min_1km_pts")
  pts <- readOGR(dsn=dsnPath, layer=ptsname)
  pts <- pts[,1]
  pts.out.name<- paste0(basin, "_", yrPath, "_LST")
  writeOGR(pts, dsn=dsnPath, layer=pts.out.name, driver="ESRI Shapefile")
```
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(307), ])
```
...
Plot orig data
```{r}
  {plot(NoNA.xyz$z, NoNA.xyz$y, xlim=c(0,366))
    abline(15,0)}
```
Plot some random reaches to take a peek
```{r}
  {plot(newnamesnum, LogPred.out[100,1:46])
...
for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.75, 0.85), from='npc'), 
      ...
      title(paste0(nameBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.9, adj=0, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Minam_2016_8D_Mx.mpeg')
```
