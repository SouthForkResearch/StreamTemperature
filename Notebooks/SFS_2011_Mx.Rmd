---
title: "South Fork Salmon 2011 Max"
...
```{r}
  ...
  nameBasin <- "South Fork Salmon" #this is from the Projects XML list of approved names
  ...
  var <- "Mx"
  longVar <- "Max"
  dataVar <- "Max8D" #name of the variable in the logger data
  capVar <- "MAX"
...
```{r, message=FALSE}
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2), validation = "LOO")
  p2 <- R2(mod2)
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(399,400), ])
```
...
```{r}  
  {plot(namesnum, LogPred.out[800,1:46])
  points(namesnum, LogPred.out[LogPred.out$RCAID == 752, 1:46], pch=16, col="blue")
  points(namesnum, LogPred.out[1234, 1:46], pch=16, col="green")
  points(namesnum, LogPred.out[562, 1:46], pch=16, col="red")
  points(namesnum, LogPred.out[635, 1:46], pch=16, col="lightblue")}
...
```{r, message=FALSE}
  ...
  error_pts@data <- error_pts@data[,c(4,9)]
  
  network <- readOGR(dsn=gisPath, layer = netname)
  network@data <- network@data[,c(11,6)]
...
```{r}
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
             y=grconvertY(c(0.35, 0.55), from='npc'), fill = attr(fix3.colors, "palette"), title="8-day Mn (°C)", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, text.col="white");
      ...
      title(paste0(nameBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=0.7, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png SFS_2011_8D_Mx.mpeg')
```
Still graphic output for wiki or web or wev
```{r}
  i <- 23  
  namey <- gsub(paste0("Tmn_", yrPath, "_"), "", colnames(network@data)[i])
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
             y=grconvertY(c(0.35, 0.55), from='npc'), fill = attr(fix3.colors, "palette"), title="8-day Mn (°C)", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, text.col="black");
      ...
      title(paste0(nameBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=0, adj=.80, col.main="black", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
