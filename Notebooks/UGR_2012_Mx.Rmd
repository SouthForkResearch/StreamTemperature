---
title: "Upper Grand Ronde 2012 Max"
...
```{r}
  ...
  LogPred.out <- LST.elev[,c(2:47,52,1)]
  LogPred.out[,1:46] <- 0
  rcas <- unique(LST.elev$RCAID)
  LST.sum <- LST.elev[,c(2:47, 52,1)]
...
```{r}
  ...
  modelPath <- paste0(mainPath, longBasin, "/", yearPath, "/")
  ...
  for (i in 3:48)
    {
      ...
      legend("topright", fill = attr(fix3.colors, "palette"), legend = c("0-4","4-8","8-10","10-12","12-14","14-16","16-18","18-20","20-22","22+"), bty = "n", cex=.5, inset=c(.05,0.1), text.col="white");
      ...
      title(paste0(longBasin, " 8-day ", var, " ", yearPath, " (°C)"), line=-0.5, adj=1, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png UGR_2012_8D_mn.mpeg')
```
...
```{r}
  ...
  filename <- paste0(mainPath, "/graphics/", basin, "_", yearPath, "_8D_", var, ".png", sep="")
