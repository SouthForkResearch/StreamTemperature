---
title: "Upper Grande Ronde 2015 Max"
...
Notes: Excluding siteID ORW03446-084462 from the model building because it appears to be spring-fed (summer temps are flat).
...
library(uuid)
  ...
  nameBasin <- "Upper Grande Ronde" #this is from the Projects XML list of approved names
...
```
split data into seasonal subsets
```{r}
  ind <- apply(LST.Log.out, 1, function(x) !any(is.na(x)))
  NoNA.xyz <- LST.Log.out[ind,]
  
  NoNA.xyz <- NoNA.xyz[,c(dataVar, "LST", "JulDay", "Elev", "SiteName")]
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(1310, 1362, 1365, 1385, 1397), ])
```
...
```{r}
  data.sp <- data.sp[-c(1310, 1362, 1365, 1385, 1397), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(238,356,415,507,588), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(238,356,415,507,588), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
...
```{r}  
  {plot(namesnum, LogPred.out[635,1:46])
  points(namesnum, LogPred.out[1000, 1:46], pch=16, col="blue")
  points(namesnum, LogPred.out[1234, 1:46], pch=16, col="green")
  points(namesnum, LogPred.out[562, 1:46], pch=16, col="red")
...
```{r, message=FALSE}
  ...
  ptsname <- paste0(basin, "_CHaMP_sites")
  
  error_pts <- readOGR(dsn=gisPath, ptsname)
  error_pts@data <- error_pts@data[,c(2,7)]
...
```{r, message=FALSE}
  ...
  fix4 <- classIntervals(means, n = 10, style = "fixed",fixedBreaks=c(-1,4,8,10,12,14,16,18,20,22))
  ...
  for (i in 3:48)
    {
      ...
      fix3 <- classIntervals(network@data[,i], n = 11, style = "fixed",fixedBreaks=c(-1,4,8,10,12,14,16,18, 20,22,24,30))
    ...
    #          y=grconvertY(c(0.7, 0.9), from='npc'), fill = attr(fix3.colors, "palette"), title="8-day Mn (°C)", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
    legend(x=grconvertX(c(0.8, 0.9), from='npc'), 
             y=grconvertY(c(0.72, 0.92), from='npc'), fill = attr(fix3.colors, "palette"), legend = c("0-4","4-8","8-10","10-12","12-14","14-16","16-18","18-20","20-22","22+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      legend(x=grconvertX(c(0.8, 0.9), from='npc'), 
             y=grconvertY(c(0.4, 0.55), from='npc'),, title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.25), col = "gray40", cex=.5, text.col="white");
         
      
      title(paste0(longBasin, " 8-day ", var, " ", yearPath, " (°C)"), line=-0.1, adj=0.7, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
      mtext(paste0("Julian Day ", namey),side =1, outer=TRUE, line=-5, col="white", cex=.5)
   tmp2 <- subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,30), cex.main=.8, main="Basin mean (+/-SD)", line=1, adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.1,0.45), from='npc'), 
        y=grconvertY(c(0.70, 0.85), from='npc'),
      ...
      tmp2 <- subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,30), cex.main=.8, main="Basin mean (+/-SD)", line=1, adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.1,0.45), from='npc'), 
        y=grconvertY(c(0.7, 0.85), from='npc'),
        size=c(1,1.5), vadj=0.5, hadj=0.5, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png UGR_2015_8D_Max.mpeg')
```
Still graphic output for wiki or web or wev
```{r}
  i <- 23  
  namey <- gsub(paste0("T", var, yrPath, "_"), "", colnames(network@data)[i])
  fix3 <- classIntervals(network@data[,i], n = 11, style = "fixed",fixedBreaks=c(-1,4,8,10,12,14,16,18,20,22,24))
  fix3.colors <- findColours(fix3,pal=seis)
  ...
  filename <- paste0(modelPath, "/graphics/Stills/", basin, "_", yearPath, "_8D_", var, ".png", sep="")
    ...
    legend(x=grconvertX(c(0.8, 0.9), from='npc'), 
             y=grconvertY(c(0.72, 0.92), from='npc'), fill = attr(fix3.colors, "palette"), legend = c("0-4","4-8","8-10","10-12","12-14","14-16","16-18","18-20","20-22","22+"), bty = "n", cex=.5, inset=c(.1,0), text.col="black");
      legend(x=grconvertX(c(0.9, 0.9), from='npc'), 
             y=grconvertY(c(0.4, 0.55), from='npc'),, title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.25), col = "gray40", cex=.5, text.col="black");
...
<Project xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"
  ...
  <ProjectType>STREAMTEMPMAX</ProjectType>
