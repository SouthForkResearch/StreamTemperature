...
```{r}
 rcaName <- paste0(basin, "_RCAID_IP")
    ...
    rcas <- read.csv(paste0(mainPath, "/", longBasin, "/", rcaName, ".csv"), stringsAsFactors = FALSE)
  ...
  rca <- subset(rcas, CHNK_IP == "Chinook") 
...
```{r, message=FALSE}
  ...
  #outnetname <- paste0(basin, "_", yearPath, "_8D_mx")
...
```{r}
  for (i in 49:60) #July 1-Sept 30 2011 1:12, 2012 13:24, 2013 25:36, 2014 37:48, 2015 49:60, 2016 61:72  #25:36 
    {
      
      j <- i-24  # 2011 i+24, 2012 i+12, 2013 i, 2014 i-12, 2015 i-24, 2016 i-36
    ...
    legend(x=grconvertX(c(0.84, 0.95), from='npc'),
             y=grconvertY(c(0.35, 0.65), from='npc'), 
           fill = c("#0000CD", "#C1CDCD"), title="8-day mean max", legend = c("< 18 °C", "> 18 °C"), bty = "n", cex=.6, inset=c(.1,0), text.col="white");
      
      par(oma=c(0,0,0,0))
      
      title(paste0("Available Chinook holding and spawning habitat in the ", nameBasin,", 1 July - 30 Sept, 2011-2015"), line=-0.1, adj=1.0, col.main="white", col.sub="white", outer=FALSE, cex.main=0.6, cex.sub=0.5)
      ...
      #change xlim
       tmp2 <- subplot(
        plot(sequ[1:i], dispPerc[1:i], type="l", col="lightblue", bty="n", ylim=c(0,100), xlim=c(1,72), xlab="  2011    2012    2013    2014    2015", ylab="", cex.lab=.98, cex.main=.95, main="Percent usable stream length", adj=0, line=.5, col.lab="white", col.main = "white", cex.axis=0.6, col.axis="white", bg="black", yaxp=c(0,100,2), xaxp=c(2011,2015,2)),
...
setwd(paste0(otherOutPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 15 -morph 3 *.png Sec_Chinook_18.mpeg')
```
Find the peak low for graphing
```{r}
      
      
      m <- which.min(dispPerc[1:60])
    
      # check to make sure the correct year's network is loaded

```
```{r}
      i <- 60 #length(dispPerc)
      j <- m # pull this from the appropriate year 2011 i+24, 2012 i+12, 2013 i, 2014 i-12, 2015 i-24, 2016 i-36
                  # 2011 1:12, 2012 13:24, 2013 25:36, 2014 37:48, 2015 49:60, 2016 61:72  
    ...
    legend(x=grconvertX(c(0.84, 0.99), from='npc'),
      ...
      mtext(paste0(yearPath),side =1, outer=TRUE, line=-3, col="black", cex=.75)
       ...
       tmp2 <- subplot(
        plot(sequ[1:i], dispPerc[1:i], type="l", col="blue", bty="n", ylim=c(0,100), xlim=c(1,72), xlab="     2011    2012    2013    2014    2015", ylab="", cex.lab=.82, cex.main=.8, main="Percent usable stream length", adj=0, line=.5, col.lab="black", col.main = "black", cex.axis=0.6, col.axis="black", bg="white", yaxp=c(0,100,2), xaxp=c(2011,2015,2)),
        #axis(1, at=1:6, labels = c("2011", "2012", "2013", "2014", "2015", "2016")),
        
        x=grconvertX(c(0.02,0.40), from='npc'), 
        y=grconvertY(c(0.02, 0.20), from='npc'),
        size=c(1,1.5), vadj=0.5, hadj=0.5, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
...
```{r}
    ...
    j <- 60 #change for different number of years 
  ...
  plot(sequ[1:j], dispPerc[1:j], type="l", col="blue", bty="n", ylim=c(0,110), xlim=c(1,j),
       
       xlab="      2011     2012      2013      2014      2015", ylab="", cex.lab=.65, cex.main=.4, 
       ...
       cex.axis=0.5, col.axis="black", bg="white", yaxp=c(0,100,2), xaxp=c(2011,2015,2))
