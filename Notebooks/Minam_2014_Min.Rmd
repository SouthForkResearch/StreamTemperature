...
{plot(1:46, CLST.out[1149,1:46])
  points(1:46, CLST.out[100, 1:46], pch=16, col="blue")
...
{plot(1:46, rca_zonal[rca_zonal$RCAID==1436,1:46])
  ...
  points(1:46, rca_zonal[1, 1:46], pch=16, col="red")
  points(1:46, rca_zonal[800, 1:46], pch=16, col="lightblue")}
```
Sub data for neighbor RCAs for some RCAs with erroneous data
```{r}
rca_zonal[rca_zonal$RCAID==636,1:46] <- rca_zonal[rca_zonal$RCAID==638,1:46]
rca_zonal[rca_zonal$RCAID==1151,1:46] <- rca_zonal[rca_zonal$RCAID==638,1:46]
rca_zonal[rca_zonal$RCAID==1436,1:46] <- rca_zonal[rca_zonal$RCAID==638,1:46]
rca_zonal[rca_zonal$RCAID==721,1:46] <- rca_zonal[rca_zonal$RCAID==723,1:46]
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(1), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
```{r}  
  ...
  points(namesnum, LogPred.out[1434, 1:46], pch=16, col="blue")
  points(namesnum, LogPred.out[640, 1:46], pch=16, col="green")
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Minam_2014_8D_Min.mpeg')
```
