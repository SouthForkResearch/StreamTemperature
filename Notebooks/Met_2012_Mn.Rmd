...
```{r}
  ...
  write.csv(x=NoNA.xyz, file= paste0(basin, "_", yearPath, "_", var, "_model_data.csv"), row.names = FALSE)
  NoNA.xyz <- read.csv(paste0(basin, "_", yearPath, "_", var, "_model_data.csv"), stringsAsFactors = FALSE)
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(86), ])
```
...
```{r}
  ...
  NoNA.pred <- NoNA.pred[,c("Avg8DayAvg", "Pred", "JulDay", "SiteName")]
...
```{r, message=FALSE}
  ...
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
      ...
      title(paste0(longBasin, " 8-day ", var, " ", yearPath, " (°C)"), line=1.0, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Methow_2012_8D_Mn.mpeg')
```
