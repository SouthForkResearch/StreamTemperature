...
{plot(1:46, CLST.out[562,1:46])
  points(1:46, CLST.out[1000, 1:46], pch=16, col="blue")
  points(1:46, CLST.out[1, 1:46], pch=16, col="green")
...
```{r}
  {plot(1:46, rca_zonal[rca_zonal$RCAID==312,1:46], ylim=c(-10,56))
    ...
    points(1:46, rca_zonal[80, 1:46], pch=16, col="lightblue")}
```
Write out rca_zonal
```{r}
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(56),])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}  
  ...
  points(namesnum, LogPred.out[LogPred.out$RCAID == 312, 1:46], pch=16, col="red")
...
```{r}
  ...
  for (i in 3:48)
    ...
    legend("topright", fill = attr(fix3.colors, "palette"), title="°C",legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(0,0), text.col="white");
      ...
      title(paste0(longBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.2, adj=0.5, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Aso_2016_8D_Mn.mpeg')
```
