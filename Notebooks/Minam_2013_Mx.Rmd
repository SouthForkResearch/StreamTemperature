...
```{r}
  ...
  for (i in SiteID) 
    { 
      ...
      eightday <- rollapply(full.year$Min, width = 8, FUN = mean, na.rm = T, fill = NA, align = c("left"))
      eightday <- as.matrix(eightday)
      full.year$Dv8D<- eightday
      full.year$Dv8D[361] <- mean(full.year$Min[361:365 ])
...
write.csv(Dv.8Day.out, paste0("../", basin, "_", yearPath, "_Min_8Day", ".csv"), row.names = FALSE)
```
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.sp[-c(32), ])
```
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Minam_2013_8D_Mx.mpeg')
```
