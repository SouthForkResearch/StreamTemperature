---
title: "South Fork Salmon 2016 Min"
...
{plot(1:46, rca_zonal[rca_zonal$RCAID==636,1:46])
  ...
  points(1:46, rca_zonal[rca_zonal$RCAID==1320, 1:46], pch=16, col="green")
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(706,725), ])
```
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(68,73), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
             y=grconvertY(c(0.35, 0.55), from='npc'), fill = attr(fix3.colors, "palette"), title="°C", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, text.col="white");
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png SFS_2016_8D_Min.mpeg')
```
