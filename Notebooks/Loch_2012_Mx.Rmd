...
```{r}
  ...
  nameBasin <- "Lochsa" #this is from the Projects XML list of approved names
...
Plot to look
```{r}
{plot(1:46, CLST.out[114,1:46])
  points(1:46, CLST.out[562, 1:46], pch=16, col="blue")
  points(1:46, CLST.out[100, 1:46], pch=16, col="green")
  points(1:46, CLST.out[1, 1:46], pch=16, col="red")
  points(1:46, CLST.out[80, 1:46], pch=16, col="lightblue")}
```
Write it out
```{r}
...
Plot to look
```{r}
{plot(1:46, rca_zonal[rca_zonal$RCAID==3,1:46])
  points(1:46, rca_zonal[rca_zonal$RCAID==667, 1:46], pch=16, col="blue")
  points(1:46, rca_zonal[rca_zonal$RCAID==896, 1:46], pch=16, col="green")
  points(1:46, rca_zonal[rca_zonal$RCAID==345, 1:46], pch=16, col="red")
  points(1:46, rca_zonal[rca_zonal$RCAID == 200, 1:46], pch=16, col="lightblue")}
```
Write out rca_zonal
```{r}
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + e + d, data=data.sp[-c(##), ])
```
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.sp[-c(1694,1754,1761,1678,1745,1679,1627,1746,1812,1689,1756,1783,1611,1612,1760,1693,1695,1696), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}
  data.sp <- data.sp[-c(1694,1754,1761,1678,1745,1679,1627,1746,1812,1689,1756,1783,1611,1612,1760,1693,1695,1696), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(80,66), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}
  {plot(newnamesnum, LogPred.out[130,1:46])
  points(newnamesnum, LogPred.out[200, 1:46], pch=16, col="blue")
...
```{r}
  ...
  netname <- paste0(longBasin, "_STHD_net")
  ptsname <- paste0(basin, "_sites_rca")
  
  error_pts <- readOGR(dsn=gisPath, ptsname)
  error_pts@data <- error_pts@data[,c(5,19)]
...
```
# if needed to read the spatial data back in
```{r}

 finalModelPath <- paste0(mainPath, longBasin, "/", yearPath, "/", longVar)
  network <- readOGR(dsn=finalModelPath, layer = outnetname)
  error_pts <- readOGR(dsn=finalModelPath, outptsname)
  error_pts <- spTransform(error_pts, proj4string(network))
...
for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.0, 0.10), from='npc'), 
             y=grconvertY(c(0.55, 0.91), from='npc'), fill = attr(fix4.colors, "palette"), title="°C",legend = c("0-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20","20+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      
    legend(x=grconvertX(c(0.0, 0.20), from='npc'), 
             y=grconvertY(c(0.31, 0.51), from='npc'), title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.25), col = "gray40", cex=.5, text.col="white");
         
      
      title(paste0(longBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.9, adj=.5, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
   ...
   tmp2 <- subplot(
        ...
        x=grconvertX(c(0.5,0.90), from='npc'), 
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Lochsa_2012_8D_Mx.mpeg')
```
...
```{r}
  ...
  filename <- paste0(modelPath, "/graphics/Stills/", basin, "_", yearPath, "_8D_", var, ".png", sep="")
    png(filename=filename, res = 300, width = 1500, height = 1500, units = "px", bg="white")
        ...
        legend(x=grconvertX(c(0.0, 0.10), from='npc'), 
             y=grconvertY(c(0.55, 0.91), from='npc'), fill = attr(fix4.colors, "palette"), title="°C",legend = c("0-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20","20+"), bty = "n", cex=.5, inset=c(.1,0), text.col="black");
      
    legend(x=grconvertX(c(0.0, 0.20), from='npc'), 
             y=grconvertY(c(0.31, 0.51), from='npc'), title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.25), col = "gray40", cex=.5, text.col="black");
         
      
      title(paste0(longBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.9, adj=.5, col.main="black", col.sub="black", outer=FALSE, cex.main=0.7, cex.sub=0.7)
    
      mtext(paste0("Julian Day ", namey),side =1, outer=TRUE, line=-4, col="black", cex=.7)
   tmp2 <- subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,25), cex.main=.8, main="Basin mean (+/-SD)", adj=0.2, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "black", bg="white"), 
        x=grconvertX(c(0.5,0.90), from='npc'), 
        y=grconvertY(c(0.02, 0.20), from='npc'),
        size=c(1,1.5), vadj=0.5, hadj=0.5, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
