---
title: "Lower Clearwater Mean 2011"
...
```{r}
  ...
  Dv.8Day.out <- data.frame (mup = NULL)
      ...
      eightday <- rollapply(full.year$Dv_z1, width = 8, FUN = mean, na.rm = T, fill = NA, align = c("left"))
      eightday <- as.matrix(eightday)
      full.year$Dv8D_z1<- eightday
      full.year$Dv8D_z1[361] <- mean(full.year$Dv_z1[361:365 ])
      
      eightday <- rollapply(full.year$Dv_z2, width = 8, FUN = mean, na.rm = T, fill = NA, align = c("left"))
      eightday <- as.matrix(eightday)
      full.year$Dv8D_z2<- eightday
      full.year$Dv8D_z2[361] <- mean(full.year$Dv_z2[361:365 ])
...
```{r}
  ...
  colnames(LST.in)<-gsub("X11", "", colnames(LST.in))
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d1 + d2, data=data.sp[-c(1646,1722,1795,1649,1719,1576,1941), ])
```
...
```{r}
  data.sp <- data.sp[-c(1646,1722,1795,1649,1719,1576,1941), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r, message=FALSE}
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2) + z + e + d1 + d2, validation = "LOO")
  p2 <- R2(mod2)
  detach("package:pls", unload=TRUE)
```
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d1 + d2, data=data.fall[-c(1,2,139,140), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(1,2,139,140), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
...
```{r}
  ...
  error_pts@data <- error_pts@data[,c(1,3)]
  error_pts@data$Orig_ID <- as.character(error_pts@data$Orig_ID)
  ...
  for (i in 3:48)
    {
      ...
      legend("bottomright", pch=16, col="gray40", pt.cex=c(0.5, 0.75, 1.0, 1.25), title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", cex=.5, text.col="white");
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png LCW_2011_8D_Mn.mpeg')
```
