...
```{r}
  ...
  ptsname <- paste0("tuc1km_pts")
  pts <- readOGR(dsn=dsnPath, layer=ptsname)
  pts@data <- pts@data[,1]
  outPath <- paste0(dataPath, yearPath, "/")
...
Plot to look
```{r}
{plot(1:46, rca_zonal[rca_zonal$RCAID==143,1:46])
  points(1:46, rca_zonal[rca_zonal$RCAID==115, 1:46], pch=16, col="blue")
  points(1:46, rca_zonal[rca_zonal$RCAID==63, 1:46], pch=16, col="green")
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.fall[-c(639,743), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r, message=FALSE}
  ...
  namesnum <- as.numeric(gsub(paste0(varName, "_"), "", colnames(network@data[3:48])))
  ...
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
             y=grconvertY(c(0.35, 0.55), from='npc'), fill = attr(fix3.colors, "palette"), title="°C", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      ...
      title(paste0(longBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.9, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
      mtext(paste0("Julian Day ", namey),side =1, outer=TRUE, line=-4, col="white", cex=.5)
   tmp2 <- subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,22), cex.main=.8, main="Basin mean (+/-SD)", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Tuc_2015_8D_Max.mpeg')
```
