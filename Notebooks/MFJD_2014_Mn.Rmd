...
library(uuid)
  ...
  nameBasin <- "South Fork John Day" #this is from the Projects XML list of approved names
...
```{r}
  ...
  ID.in <- read.csv(paste0("../../", "JD_sites_RCA_elev.csv"), stringsAsFactors = FALSE) 
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(247,269), ])
```
...
```{r}
  ...
  elev.in <- read.csv(paste0(mainPath, longBasin, "/",  "JD_rca_elev.csv"), stringsAsFactors = F)
  colnames(elev.in)[1] <- "RCAID"
  
  LST.in <- read.dbf(paste0(mainPath, longBasin, "/", "LST", yrPath, "_JD_RCA.dbf"))
...
```{r, message=FALSE}
  ...
  gisPath <- paste0("D:/OneDrive/work/research/CHaMP/GIS/coverages/John_Day")
...
```{r, message=FALSE}
  ...
  namesnum <- as.numeric(gsub(paste0("Tmn", "_", yrPath, "_"), "", colnames(network@data[3:48])))
  ...
  for (i in 2:47)
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
             y=grconvertY(c(0.5, 0.75), from='npc'), fill = attr(fix3.colors, "palette"), title="8-day Mn (°C)", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");

      # legend(x=grconvertX(c(0.01, 0.20), from='npc'),
      ...
      title(paste0(basin, " 8-day ", var, " ", yearPath, " (°C)"), line=-0.9, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
      mtext(paste0("Julian Day ", namey),side =1, outer=TRUE, line=-4, col="white", cex=.5)
   tmp2 <- subplot(
        plot(namesnum[1:(i-1)], means[1:(i-1)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,20), cex.main=.8, main="Basin mean (+/-SD)", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png MFJD_2014_8D_Mn.mpeg')
```
...
```{r}
  ...
  namey <- gsub(paste0("T", var, "_", yrPath, "_"), "", colnames(network@data)[i])
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
             y=grconvertY(c(0.5, 0.75), from='npc'), fill = attr(fix3.colors, "palette"), title="8-day Mn (°C)", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="black");

      # legend(x=grconvertX(c(0.01, 0.2), from='npc'),
      ...
      title(paste0(midBasin, " 8-day ", var, " ", yearPath, " (°C)"), line=-0.9, adj=.80, col.main="black", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
