---
title: "Middle Fork John Day 2014 Min"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(279,294,307,333,320,346), ])
```
...
```{r}
  data.sp <- data.sp[-c(279,294,307,333,320,346), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 15 -morph 3 *.png MFJD_2014_8D_Min.mpeg')
```
