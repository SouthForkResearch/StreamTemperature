---
title: "South Umpqua Watershed 2016 Mean"
...
```{r}
  ...
  nameBasin <- "SUmpqua" #this is from the Projects XML list of approved names
  
  
  mainPath <- "D:/OneDrive/work/research/CHaMP/CHaMP_data/"
  metricPath <- "D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/metrics"
...
```{r, message=FALSE}
  ...
  LST.in <- read.csv("D:/OneDrive/work/research/CHaMP/GIS/coverages/SUmpqua/SUmpqua_LST_2016.csv")
  PointID <- LST.in[,1]
  LST.in <- LST.in[,2:47]
...
```{r}
  ...
  write.dbf(CLST.out, file = paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/LST16_UMS_interp.dbf"))
  
  
```
This part calculates the zonal mean of the daily LST for each RCA
```{r}
out_Preds <- read.dbf("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/LST16_UMS_interp.dbf")
  
  
  weights <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/UMS_rca_area_wgts.csv")
...
```{r}
  ...
  plot(1:46, rca_zonal[100,1:46])
    write.dbf(rca_zonal, file = paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/LST16_UMS_RCA.dbf"))
  
```
Discharge data formatting
```{r}

  Dv.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/UMS_dv_data_2016.csv", stringsAsFactors = FALSE)
...
write.csv(Dv.8Day.out, paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/UMS_2016_Dv_8Day", ".csv"), row.names = FALSE)
...
```{r}
  Log.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/UMS_logger_8D_2016.csv", stringsAsFactors = FALSE)
 

  ID.in <- read.csv("D:/OneDrive/work/research/CHaMP/GIS/coverages/SUmpqua/UMS_sites_rca_2016.csv", stringsAsFactors = FALSE) 
  
  LST.in <- read.dbf(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/LST16_UMS_RCA.dbf"))
  colnames(LST.in)<-gsub("X", "", colnames(LST.in))
  newnamesnum <- as.numeric(colnames(LST.in)[1:46]);
  Dv.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/UMS_2016_Dv_8Day.csv", stringsAsFactors = FALSE)
...
Spring data model
```{r}
  coeffs_out <- data.frame(Int=numeric(2), bLST=numeric(2), bLST2=numeric(2), bJul=numeric(2), bElev=numeric(2), bD=numeric(2), bDJul=numeric(2), bze=numeric(2))
  metrics_out <- data.frame(r2=numeric(2), RMSE=numeric(2), p2=numeric(2), RMSEP=numeric(2), N_Sites=numeric(2), N=numeric(2))
  rownames(metrics_out) <- c("Spring", "Fall")
  rownames(coeffs_out) <- c("Spring", "Fall")
```
...
```{r, message=FALSE}
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2) + z + e + d + z*e, validation = "LOO")
  p2 <- R2(mod2)
  detach("package:pls", unload=TRUE)
```
...
```{r}
  ...
  elev.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/UMS_rca_elev.csv"), stringsAsFactors = F)
   ...
   for (i in 1:length(rcas))  
      {
        ...
        j <- 1
        for (l in 1:midrow)
        {x[l] <- x[l] * coeffs.in$bLST[1] + j * coeffs.in$bJul[1] +  x[47] * coeffs.in$bElev[1] + Dv[l] * coeffs.in$bD[1] + x[l]^2 * coeffs.in$bLST2[1] + j*Dv[l]*coeffs.in$bDJul[1] + x[47] *j*coeffs.in$bze[1]+coeffs.in$Int[1]
        j <- j + 8}
        k <- day
        for (l in maxrow:46)     
        {x[l] <- x[l] * coeffs.in$bLST[2] + k * coeffs.in$bJul[2] +  x[47] * coeffs.in$bElev[2] + Dv[l] * coeffs.in$bD[2] + x[l]^2 * coeffs.in$bLST2[2] + j*Dv[l]*coeffs.in$bDJul[2] +  x[47] *j*coeffs.in$bze[2]+coeffs.in$Int[2]
        k <- k + 8}
        if (maxrow > 3)
        {
          x[(midrow)] <- NA
...
```{r}  
  ...
  points(namesnum, LogPred.out[10000, 1:46], pch=16, col="green")
...
```{r}
  ...
  Log.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/SUmpqua/2016/UMS_logger_8D_2016.csv"), stringsAsFactors = FALSE)
  
  ID.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/GIS/coverages/SUmpqua/UMS_sites_rca_2016.csv"), stringsAsFactors = FALSE) 
...
```{r}
  ...
  netname <- paste0("SUmpqua_NSI_2016")
  ptsname <- paste0("UMS_TempSite_2016")
  ...
  for (i in 3:48)
    {
      ...
      points(error_pts, pch=16, col="gray40", cex=cexEr)
       ...
       legend(x=grconvertX(c(0.85,0.9), from='npc'), 
        y=grconvertY(c(0.01, 0.46), from='npc'), fill = c("#0024E3", "#0087CD", "#16F45A", "#3FFA36", "#BDFF0C", "#FFDD00", "#FF9000", "#FF4400", "#F70000", "#AA0000", "#910000","#770000","#5e0000", "#480000"), legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20", "20-22","22-24","2-26","26"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      ...
      tmp2 <- subplot(
        ...
        x=grconvertX(c(0.40,.80), from='npc'), 
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png UMS_2016_8D_Mn.mpeg')
```
