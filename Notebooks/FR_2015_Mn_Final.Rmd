...
```{r}
  ...
  metricPath <- "D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/2015/metrics"
...
```{r}
  ...
  write.dbf(CLST.out, file = paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/2015/LST15_FR_interp.dbf"))
  
  
```
This part calculates the zonal mean of the daily LST for each RCA
```{r}
out_Preds <- read.dbf("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/2015/LST15_FR_interp.dbf")
...
```{r}
  ...
  write.dbf(rca_zonal, file = paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/2015/LST15_FR_RCA.dbf"))
  
```
Discharge data formatting
```{r}

  Dv.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_Data/Feather/2015/FR_dv_data_2015.csv", stringsAsFactors = FALSE)
...
write.csv(Dv.8Day.out, paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/2015/FR_2015_Dv_8Day", ".csv"), row.names = FALSE)
...
```{r}
  Log.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/2015/FR_logger_8D_2015.csv", stringsAsFactors = FALSE)
 

  ID.in <- read.csv("D:/OneDrive/work/research/CHaMP/GIS/coverages/Feather_River/FR_sites_rca_2015_tribs.csv", stringsAsFactors = FALSE) 
  
  LST.in <- read.dbf(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/2015/LST15_FR_RCA.dbf"))
  colnames(LST.in)<-gsub("X", "", colnames(LST.in))
  newnamesnum <- as.numeric(colnames(LST.in)[1:46]);
  Dv.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/2015/FR_2015_Dv_8Day.csv", stringsAsFactors = FALSE)
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.sp[-c(30), ])
```
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(1028), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.fall[-c(14), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```{r}
  ...
  Log.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/2015/FR_logger_8D_2015.csv"), stringsAsFactors = FALSE)
  
  ID.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/GIS/coverages/Feather_River/FR_sites_rca_2015_tribs.csv"), stringsAsFactors = FALSE) 
...
```{r}
  ...
  ptsname <- paste0("TempSites_2015_tribs")
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png FR_2015_8D_Mn.mpeg')
```
