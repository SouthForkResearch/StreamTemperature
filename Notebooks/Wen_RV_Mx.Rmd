---
title: "Wenatchee RV modeling - Max temp"
...
```{r}
  ...
  baw <- read.csv(paste0(basin, "_BAW_RCAID.csv"), stringsAsFactors = FALSE)
                  
  bps <- read.csv(paste0(basin, "_BpS_RCAID.csv"), stringsAsFactors=FALSE)
                  
  rca_zonal <- merge(baw, bps, by="RCAID")
  
  colnames(rca_zonal)[2:3] <- c("Cur_EVH", "Pot_EVH")
```
