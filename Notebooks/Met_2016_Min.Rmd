...
{plot(1:46, CLST.out[562,1:46])
  points(1:46, CLST.out[1149, 1:46], pch=16, col="blue")
...
```{r}  
  {plot(namesnum, LogPred.out[LogPred.out$RCAID == 93,1:46])
  points(namesnum, LogPred.out[LogPred.out$RCAID == 45, 1:46], pch=16, col="blue")
  points(namesnum, LogPred.out[LogPred.out$RCAID == 1936, 1:46], pch=16, col="green")
...
```{r, message=FALSE}
  ...
  ptsname <- paste0("met_sites_rca")
  
  error_pts <- readOGR(dsn=gisPath, ptsname)
  error_pts@data <- error_pts@data[,c(4,15)]
...
```{r, message=FALSE}
  ...
  newnamesnum <- gsub(paste0(varName, "_"), "", colnames(network@data[3:48]))
  ...
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.88, 0.95), from='npc'),
      ...
      title(paste0(longBasin, " 8-day ", var, " ", yearPath, " (°C)"), line=.5, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Met_2016_8D_Min.mpeg')
```
