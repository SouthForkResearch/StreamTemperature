...
library(uuid)
  ...
  basin <- "YF"
  midBasin <- "YankeeFork"
  longBasin <- "Yankee_Fork"
  yrPath <- "15"
  yearPath <- "2015"
  
  nameBasin <- "Yankee Fork" #this is from the Projects XML list of approved names
  
  dataPath <- "D:/OneDrive/work/GIS/LST/LST_s12_"
...
```{r}
  ...
  write.dbf(CLST.out, file = paste0(mainPath, midBasin, "/", "LST", yrPath, "_", basin, "_interp.dbf"))
  
  
```
This part calculates the zonal mean of the daily LST for each RCA
```{r}
out_Preds <- read.dbf(paste0(mainPath, midBasin, "/", "LST", yrPath, "_", basin, "_interp.dbf"))
...
```{r}
  ...
  for (i in SiteID) 
    { 
      ...
      eightday <- rollapply(full.year$mean_va, width = 8, FUN = mean, na.rm = T, fill = NA, align = c("left"))
      eightday <- as.matrix(eightday)
      full.year$Dv8D<- eightday
      full.year$Dv8D[361] <- mean(full.year$mean_va[361:365 ])
...
```{r}
  ...
  Flow.in <- read.csv(paste0("../../", basin, "_flow_rca.csv"), stringsAsFactors = FALSE)
  Dv.in <- read.csv(paste0("../", basin, "_", yearPath, "_Dv_8Day", ".csv"), stringsAsFactors = FALSE)
  Dv.in <- Dv.in[,c("Dv8D", "Days")]
  ID.in <- merge(ID.in, Flow.in, by = "RCAID")
  ...
  NoNA.xyz <- NoNA.xyz[,c(dataVar, "LST", "JulDay", "Elev", "Flow", "Dv8D", "SiteName")]
  colnames(NoNA.xyz) <- c("y", "x", "z", "e", "q", "d", "SiteName")
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(130), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
```{r}
  ...
  Flow.in <- Flow.in[,c("RCAID", "MAFLOWV")]
  LST.flow <- merge(LST.in, Flow.in, by = "RCAID")
  LST.elev <- merge(LST.flow, elev.in, by="RCAID") 
  
  coeffs.in <- read.csv(paste0("All_data_", basin, "_", yearPath, "_mod_coeffs_", var, ".csv"), stringsAsFactors=FALSE)
  LogPred.out <- LST.elev[,c(2:49, 1)]
  LogPred.out[,1:46] <- 0
  rcas <- unique(LST.flow$RCAID)
  LST.sum <- LST.elev[,c(2:49, 1)]
    ...
    for (i in 1:length(rcas))  
      {
        ...
        j <- 1
        for (l in 1:midrow)
        {x[l] <- x[l] * coeffs.in$bLST[1] + j * coeffs.in$bJul[1] +  x[48] * coeffs.in$bE[1] + Dv[l] * coeffs.in$bD[1] + x[l]^2 * coeffs.in$bLST2[1] +  + coeffs.in$Int[1]
        j <- j + 8}
        k <- day
        for (l in maxrow:46)     
        {x[l] <- x[l] * coeffs.in$bLST[2] + k * coeffs.in$bJul[2] +  x[48] * coeffs.in$bE[2] + Dv[l] * coeffs.in$bD[2] + x[l]^2 * coeffs.in$bLST2[2] + coeffs.in$Int[2]
        k <- k + 8}
        if (maxrow > 3)
        {
          x[(midrow)] <- NA
...
```{r}
  ...
  netname <- paste0(basin, "_RCA_net")
  ptsname <- paste0(basin, "_sites_rca")
  
  error_pts <- readOGR(dsn=gisPath, ptsname)
  error_pts@data <- error_pts@data[,c("SiteName", "rca_id")]
  
  network <- readOGR(dsn=gisPath, layer = netname)
  network@data <- network@data[,c("REACHCODE", "rca_id")]
...
```{r}
  ...
  rcaName <- paste0(basin, "_", "RCAID_net")
    ...
    if (ext != "Full") {
      ...
      ANY <- apply(ind, 2, any)
      rcasTest <- rcas[ANY,]
      rcaList <- unique(rcasTest$rca_id)
    } else{
      rcaList <- rcas$rca_id
    }

    net = network[network$rca_id %in% rcaList,]
    network = net   
```
```{r}

  network@data$REACHCODE <- as.numeric(network@data$REACHCODE)
  ...
  for (i in 3:48)
    {
      ...
      namey <- gsub(paste0("T", var, "_", yrPath, "_"), "", colnames(net@data)[i])
      ...
      points(error_pts, pch=16, col="gray40", cex=cexEr)
       ...
       legend(x=grconvertX(c(0.85,0.9), from='npc'), 
        y=grconvertY(c(0.3, 0.55), from='npc'),, fill = attr(fix3.colors, "palette"), title=paste0("8-day ", longVar, " (°C)"),legend = c("0-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20", "20-22", "22+"), bty = "n", cex=.5, inset=c(0,0), text.col="white");
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png YF_2015_8D_Mn.mpeg')
```
