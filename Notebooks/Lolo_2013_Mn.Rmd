...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.sp[-c(287), ])
```
...
for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.89, 0.99), from='npc'), 
             y=grconvertY(c(0.30, 0.55), from='npc'), fill = attr(fix4.colors, "palette"), title="°C",legend = c("0-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20","20+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Lolo_2013_8D_Mn.mpeg')
```
