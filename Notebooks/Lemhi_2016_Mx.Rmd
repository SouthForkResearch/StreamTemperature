...
Switched from USGS site 13305000 to site 13305310 (13305000 was iced for part of 2016, so using data from a gauge further downstream)
...
```{r}
  ...
  nameBasin <- "Lemhi" #this is from the Projects XML list of approved names
...
{plot(1:46, CLST.out[1000,1:46])
  ...
  points(1:46, CLST.out[1349, 1:46], pch=16, col="green")
...
```{r}  
  {plot(namesnum, LogPred.out[LogPred.out$RCAID == 1160,1:46])
  points(namesnum, LogPred.out[LogPred.out$RCAID == 1815, 1:46], pch=16, col="blue")
  points(namesnum, LogPred.out[1234, 1:46], pch=16, col="green")
  points(namesnum, LogPred.out[1936, 1:46], pch=16, col="red")
...
```{r, message=FALSE}
  ...
  ptsname <- paste0(basin, "_all_sites_RCAID")
  
  error_pts <- readOGR(dsn=gisPath, ptsname)
  error_pts@data <- error_pts@data[,c(5,15)]
  
  network <- readOGR(dsn=gisPath, layer = netname)
  network@data <- network@data[,c(33,6)]
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Lemhi_2016_8D_Mx.mpeg')
```
