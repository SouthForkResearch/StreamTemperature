---
title: "Upper Grande Ronde 2016 Max"
...
```{r}
  ...
  dsnPath <- (paste0(gisPath, "/", midBasin))
  
  ptsname <- paste0("gr1km_pt")
...
{plot(1:46, CLST.out[1000,1:46])
  ...
  points(1:46, CLST.out[1149, 1:46], pch=16, col="green")
...
out_Preds <- read.dbf(paste0(mainPath, longBasin, "/", "LST", yrPath, "_", basin, "_interp.dbf"))
  ...
  weights <- read.dbf(paste0(mainPath, longBasin, "/", basin, "_rca_area_wgts.dbf"))
...
Plot to look
```{r}
{plot(1:46, rca_zonal[rca_zonal$RCAID==636,1:46])
  points(1:46, rca_zonal[rca_zonal$RCAID==1151, 1:46], pch=16, col="blue")
  points(1:46, rca_zonal[rca_zonal$RCAID==800, 1:46], pch=16, col="green")
  points(1:46, rca_zonal[rca_zonal$RCAID==562, 1:46], pch=16, col="red")
  points(1:46, rca_zonal[1367, 1:46], pch=16, col="lightblue")}
```
Write out rca_zonal
```{r}
...
```{r}
  ...
  NoNA.xyz <- NoNA.xyz[NoNA.xyz$SiteName != "ORW03446-084462",]
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(1228,1100,1164), ])
```
...
```{r}
  data.sp <- data.sp[-c(1228,1100,1164), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.fall[-c(1,180,358,302,117), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(1,180,358,302,117), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
...
```{r}
  ...
  LST.sum <- LST.elev[,c(2:47, 48,1)]
   ...
   for (i in 1:length(rcas))  
      {
        x <- unlist(LST.sum[i,])
        maxrow <- 28 #as.numeric(which.max(x[1:46]))
        midrow <- maxrow - 1
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png UGR_2016_8D_Mx.mpeg')
```
