---
title: "Middle Fork John Day 2016 Mean"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(303,331,317,275), ])
```
...
```{r}
  data.sp <- data.sp[-c(303,331,317,275), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  {plot(newnamesnum, LogPred.out[500,1:46])
  points(newnamesnum, LogPred.out[300, 1:46], pch=16, col="blue")
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 15 -morph 3 *.png MFJD_2016_8D_Mn.mpeg')
```
