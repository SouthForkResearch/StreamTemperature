...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=NoNA.xyz[-c(##), ])
```
...
```{r}  
  ...
  points(namesnum, LogPred.out[143, 1:46], pch=16, col="blue")
  points(namesnum, LogPred.out[340, 1:46], pch=16, col="green")
  points(namesnum, LogPred.out[435, 1:46], pch=16, col="red")
  points(namesnum, LogPred.out[200, 1:46], pch=16, col="lightblue")}
...
```{r, message=FALSE}
  ...
  error_pts@data <- error_pts@data[,c(4,6)]
  ...
  network <- merge(network, Pred.in, by.x = "rca_id", by.y = "RCAID")                    
  varName <- paste0("T", var, yrPath)
  names.out <- sprintf("%s_%03d", varName, newnamesnum)
  colnames(network@data)[3:48] <- names.out  
...
```
If the points and network already exist, but going back later to output graphics
```{r}
...
```{r, message=FALSE}
  ...
  for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.85, 0.99), from='npc'), 
             y=grconvertY(c(0.85, 0.99), from='npc'), fill = attr(fix3.colors, "palette"), title="°C", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      legend("bottomright", title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.25), col = "gray40", cex=.5, text.col="white");
   ...
   tmp2 <- subplot(
        ...
        x=grconvertX(c(0.00,0.25), from='npc'), 
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Asotin_2012_8D_Max.mpeg')
```
...
```{r}
  ...
  filename <- paste0(modelPath, "/graphics/Stills/", basin, "_", yearPath, "_8D_", var, ".png", sep="")
    png(filename=filename, res = 300, width = 1500, height = 1500, units = "px", bg="white")
      ...
      legend("topright", fill = attr(fix3.colors, "palette"), title="8-day Mean (°C)",legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(0,0), text.col="black");
      ...
      mtext("Asotin 8-day mean (°C)",side =3, outer=TRUE, line=-3, col="black", cex=.7)
      mtext(paste0("Julian Day ", namey),side =1, outer=TRUE, line=-3, col="black", cex=.7)
      
      tmp2 <- subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, main="Basin mean", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="black", col.main = "black", bg="white"), 
