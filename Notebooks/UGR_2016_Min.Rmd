---
title: "Upper Grande Ronde 2016 Min"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(1649,1471,1593,1408,1352), ])
```
...
```{r}
  data.sp <- data.sp[-c(1649,1471,1593,1408,1352), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.fall[-c(##), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png UGR_2016_8D_Min.mpeg')
```
