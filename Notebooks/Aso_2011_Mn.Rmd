...
```{r}
  ...
  NoNA.xyz <- NoNA.xyz[,c("Avg8Day", "LST", "JulDay", "Elev", "SiteName")]
  colnames(NoNA.xyz) <- c("y", "x", "z", "e", "SiteName")
  
  write.csv(x=NoNA.xyz, file= paste0(basin, "_", yearPath, "_8Day_", "model_data.csv"), row.names = FALSE)
  NoNA.xyz <- read.csv(paste0(basin, "_", yearPath, "_8Day_", "model_data.csv"), stringsAsFactors = FALSE)
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(125,131), ])
```
...
```{r, message=FALSE}
  ...
  error_pts@data <- error_pts@data[,c(3,14)]
