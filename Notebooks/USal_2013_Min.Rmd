...
library(classInt)
  ...
  modelPath <- paste0(mainPath, "USal/", basin, "/", yearPath, "/", longVar, "/")
...
```{r}
  ...
  y <- NoNA.xyz$y[-c(279,288,231)]
  x <- NoNA.xyz$x[-c(279,288,231)]
  z <- NoNA.xyz$z[-c(279,288,231)]
  e <- NoNA.xyz$e[-c(279,288,231)]
  mod <- lm(y ~ x + I(x^2) + e)
  sum_mod <- summary(mod)
```
```{r}
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(221,231), ])
```
...
```
Fill them with spring model data
```{r}
  coeffs_out[1,1] <- coeffs[1,1]
  coeffs_out[1,2] <- coeffs[2,1]
  coeffs_out[1,3] <- coeffs[3,1]
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png USal_2013_8D_Min.mpeg')
```
