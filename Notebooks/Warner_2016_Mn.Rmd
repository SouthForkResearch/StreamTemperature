---
title: "Warner Watershed 2016 Mean"
...
Created: 01 May 2019
Updated: 05 Jun 2019
USGS station: 10396000 (** not in Warner, about 70km to the east but closest station w/ available data)
...
```{r}
  ...
  nameBasin <- "Warner" #this is from the Projects XML list of approved names
  
  
  mainPath <- "D:/OneDrive/work/research/CHaMP/CHaMP_data/"
  metricPath <- "D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/metrics"
...
```{r, message=FALSE}
  ...
  LST.in <- read.csv("D:/OneDrive/work/research/CHaMP/GIS/coverages/Warner/Warner_16_LST.csv")
  PointID <- LST.in[,1]
  LST.in <- LST.in[,2:47]
...
```{r}
  ...
  write.dbf(CLST.out, file = paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/LST16_WR_interp.dbf"))
  
  
```
This part calculates the zonal mean of the daily LST for each RCA
```{r}
out_Preds <- read.dbf("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/LST16_WR_interp.dbf")
  
  
  weights <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/WR_rca_area_wgts.csv")
...
```{r}
  ...
  plot(1:46, rca_zonal[100,1:46])
    write.dbf(rca_zonal, file = paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/LST16_WR_RCA.dbf"))
  
```
Discharge data formatting
```{r}

  Dv.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/WR_dv_data_2016.csv", stringsAsFactors = FALSE)
...
write.csv(Dv.8Day.out, paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/WR_2016_Dv_8Day", ".csv"), row.names = FALSE)
...
```{r}
  Log.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/WR_logger_8D_2016.csv", stringsAsFactors = FALSE)
 

  ID.in <- read.csv("D:/OneDrive/work/research/CHaMP/GIS/coverages/Warner/WR_sites_rca_2016.csv", stringsAsFactors = FALSE) 
  
  LST.in <- read.dbf(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/LST16_WR_RCA.dbf"))
  colnames(LST.in)<-gsub("X", "", colnames(LST.in))
  newnamesnum <- as.numeric(colnames(LST.in)[1:46]);
  Dv.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/WR_2016_Dv_8Day.csv", stringsAsFactors = FALSE)
...
```
plot the full year of predicted v observed **Updated for Full Year Model
```{r}
...
```{r}
  ...
  elev.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/WR_rca_elev.csv"), stringsAsFactors = F)
...
```{r}
  ...
  Log.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Warner/2016/WR_logger_8D_2016.csv"), stringsAsFactors = FALSE)
  
  ID.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/GIS/coverages/Warner/WR_sites_rca_2016.csv"), stringsAsFactors = FALSE) 
...
```{r}
  ...
  netname <- paste0("Warner_NSI_2016")
  ptsname <- paste0("WR_TempSite_2016")
  ...
  for (i in 3:48)
    {
      ...
      points(error_pts, pch=16, col="gray40", cex=cexEr)
       ...
       legend(x=grconvertX(c(0.85,0.9), from='npc'), 
        y=grconvertY(c(0.3, 0.8), from='npc'), fill = c("#0024E3", "#0087CD", "#16F45A", "#3FFA36", "#BDFF0C", "#FFDD00", "#FF9000", "#FF4400", "#F70000", "#AA0000", "#910000","#770000","#5e0000", "#480000"), legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20", "20-22","22-24","2-26","26"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      ...
      tmp2 <- subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col=fix4.colors, pch=16, bty="n", xlim=c(175,360), ylim=c(0,25), cex.main=.8, main="Basin mean (+/-SD)", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.01,0.41), from='npc'), 
      ...
      tmp2 <- subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col=fix4.colors, pch=16, bty="n", xlim=c(175,360), ylim=c(0,25), cex.main=.8, main="Basin mean (+/-SD)", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.01,.41), from='npc'), 
        y=grconvertY(c(0.05, 0.20), from='npc'),
        size=c(1,1.5), vadj=0.5, hadj=0.5, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png WR_2016_8D_Mn.mpeg')
```
