...
Added the temp data from the USGS station to the logger dataset.  It's near site CBW05583-050623, so used that ID to place it since that site was not sampled

USGS station: # 13331500
Stage setting:
```{r, message=FALSE}
library(timeSeries)
library(lattice)
...
```{r}
  ...
  nameBasin <- "Minam" #this is from the Projects XML list of approved names
  ...
  var <- "Mx"
  longVar <- "Max"
  dataVar <- "X8DayMax" #name of the variable in the logger data
  capVar <- "MAX"
...
```{r, message=FALSE}
  ...
  LST.in <- read.dbf(paste0(gisPath, "/", longBasin, "/", longBasin, "_", yrPath, "_LST.dbf"))
  PointID <- LST.in[,1]
  LST.in <- LST.in[,2:47]
...
out_Preds <- read.dbf(paste0(mainPath, longBasin, "/", "LST", yrPath, "_", basin, "_interp.dbf"))
  ...
  weights <- read.csv(paste0(mainPath, longBasin, "/", midBasin, "_rca_area_wgts.csv"), stringsAsFactors=FALSE)
...
Plot to look
```{r}
{plot(1:46, rca_zonal[rca_zonal$RCAID==1436,1:46])
  points(1:46, rca_zonal[rca_zonal$RCAID==1151, 1:46], pch=16, col="blue")
  points(1:46, rca_zonal[rca_zonal$RCAID==636, 1:46], pch=16, col="green")
  points(1:46, rca_zonal[rca_zonal$RCAID==562, 1:46], pch=16, col="red")
...
```
Daily to 8-day logger data formatting
```{r}
  Log.in <- read.csv(paste0(mainPath, longBasin, "/", basin, "_screwtrap_logger_daily.csv"), stringsAsFactors = FALSE)
  ...
  Log.8Day.out <- data.frame (mup = NULL)
    ...
    for (i in SiteID) 
      { 
        ...
        eightday <- rollapply(full.year$DailyMax, width = 8, FUN = mean, na.rm = T, fill = NA, align = c("left"))
        eightday <- as.matrix(eightday)
        full.year$Mx8D<- eightday
        full.year$Mx8D[361] <- mean(full.year$DailyMax[361:365])
...
write.csv(Log.8Day.out, paste0(mainPath, longBasin, "/", basin, "_screwtrap_logger_8DMax_", yearPath, ".csv"), row.names = FALSE)
```
# USGS station Daily to 8-day logger data formatting (repeat for each metric)
```{r}
  ...
  for (i in SiteID) 
    { 
      ...
      eightday <- rollapply(full.year$Max, width = 8, FUN = mean, na.rm = T, fill = NA, align = c("left"))
      eightday <- as.matrix(eightday)
      full.year$Dv8D<- eightday
      full.year$Dv8D[361] <- mean(full.year$Max[361:365 ])
...
write.csv(Dv.8Day.out, paste0("../", basin, "_", yearPath, "_Mx_8Day", ".csv"), row.names = FALSE)
```
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(182,187), ])
```
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(48), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
```
Plot some random reaches to take a peek
```{r}
  {plot(newnamesnum, LogPred.out[160,1:46])
  points(newnamesnum, LogPred.out[1300, 1:46], pch=16, col="blue")
  points(newnamesnum, LogPred.out[300, 1:46], pch=16, col="green")
  points(newnamesnum, LogPred.out[100, 1:46], pch=16, col="red")
...
for (i in 3:48)
    ...
    legend(x=grconvertX(c(0.7, 0.85), from='npc'), 
             y=grconvertY(c(0.72, 0.92), from='npc'), fill = attr(fix4.colors, "palette"), title="°C",legend = c("0-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20","20+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      
    legend(x=grconvertX(c(0.75, 0.85), from='npc'), 
             y=grconvertY(c(0.0, 0.20), from='npc'), title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.25), col = "gray40", cex=.5, text.col="white");
         
      
      title(paste0(longBasin, " 8-day ", longVar, " ", yearPath, " (°C)"), line=-0.9, adj=0, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
    
      mtext(paste0("Julian Day ", namey),side =1, outer=TRUE, line=-4, col="white", cex=.5)
   tmp2 <- subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,25), cex.main=.8, main="Basin mean (+/-SD)", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.00,0.40), from='npc'), 
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Minam_2014_8D_Mx.mpeg')
```
...
```{r}
  ...
  filename <- paste0(modelPath, "/graphics/Stills/", basin, "_", yearPath, "_8D_", var, ".png", sep="")
    ...
    legend(x=grconvertX(c(0.7, 0.85), from='npc'), 
             y=grconvertY(c(0.72, 0.92), from='npc'), fill = attr(fix3.colors, "palette"), title="°C", legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="black");
      legend(x=grconvertX(c(0.05, 0.25), from='npc'), 
             y=grconvertY(c(0.3, 0.5), from='npc'), title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.25), col = "gray40", cex=.5, text.col="black");
          
        mtext(paste0(longBasin, " ",  yearPath, " 8-day ", longVar, " (°C)"),side =3, outer=TRUE, line=-3, col="black", cex=.7)
