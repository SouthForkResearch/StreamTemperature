---
title: "Lower Clearwater Mean 2012"
...
Edited 1 Aug 2017 to add discharge parameters
...
```{r}
  ...
  Dv.in <- read.csv(paste0("../", basin, "_z2_dv_data_", yearPath, ".csv"), stringsAsFactors = FALSE)
...
write.csv(Dv.8Day.out, paste0("../", basin, "_", yearPath, "_z2_Dv_8Day", ".csv"), row.names = FALSE)
...
```{r}
  ...
  colnames(LST.in)<-gsub("X12", "", colnames(LST.in))
  newnamesnum <- as.numeric(colnames(LST.in)[1:46]);
  
  Dv1.in <- read.csv(paste0("../", basin, "_", yearPath, "_Dv_8Day", ".csv"), stringsAsFactors = FALSE)
  colnames(Dv1.in)[6] <- "Dv8Dz1"
  Dv1.in <- Dv1.in[,c("Dv8Dz1", "Days")]
  
  Dv2.in <- read.csv(paste0("../", basin, "_", yearPath, "_z2_Dv_8Day", ".csv"), stringsAsFactors = FALSE)
  colnames(Dv2.in)[6] <- "Dv8Dz2"
  Dv2.in <- Dv2.in[,c("Dv8Dz2", "Days")]
  ...
  Dv1 <- unlist(LST.Log.site$Dv8Dz1)
  Dv2 <- unlist(LST.Log.site$Dv8Dz2)
  ind <- apply(LST.Log.out, 1, function(x) !any(is.na(x)))
  NoNA.xyz <- LST.Log.out[ind,]
  
  NoNA.xyz <- NoNA.xyz[,c(dataVar, "LST", "JulDay", "Elev", "Dv8Dz1", "Dv8Dz2", "SiteID")]
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d1 + d2, data=data.sp[-c(1859,1856, 1315), ])
```
...
```{r}
  data.sp <- data.sp[-c(1859,1856, 1315), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d1 + d2, data=data.fall[-c(76), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png LCW_2012_8D_Mn.mpeg')
```
