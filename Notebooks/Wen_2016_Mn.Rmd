...
```{r}
  ...
  nameBasin <- "Wenatchee" #this is from the Projects XML list of approved names
...
out_Preds <- read.dbf(paste0(mainPath, longBasin, "/", "LST", yrPath, "_", basin, "_interp.dbf"))
  ...
  colnames(weights)[1:2] <- c("RCAID", "GRIDCODE")
...
Plot to look
```{r}
{plot(1:46, rca_zonal[rca_zonal$RCAID==158,1:46])
  points(1:46, rca_zonal[rca_zonal$RCAID==56, 1:46], pch=16, col="blue")
  points(1:46, rca_zonal[rca_zonal$RCAID==63, 1:46], pch=16, col="green")
  points(1:46, rca_zonal[rca_zonal$RCAID==944, 1:46], pch=16, col="red")
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d + d*z, data=data.sp[-c(411), ])
```
...
```{r}  
  {plot(namesnum, LogPred.out[158,1:46])
  ...
  points(namesnum, LogPred.out[994, 1:46], pch=16, col="lightblue")}
...
```{r}
  ...
  LogPred.out <- read.dbf(paste0("predt", yearPath, "_", basin, "_8D_Mn.dbf"))
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Wen_2016_8D_Mn.mpeg')
```
