...
```{r, message=FALSE}
  ...
  LST.in <- read.dbf(paste0(gisPath, "/", longBasin, "_", yrPath, "_LST.dbf"))
  PointID <- LST.in[,1]
  LST.in <- LST.in[,2:47]
...
{plot(1:46, CLST.out[114,1:46])
  ...
  points(1:46, CLST.out[13, 1:46], pch=16, col="green")
...
{plot(1:46, rca_zonal[rca_zonal$RCAID==143,1:46])
  ...
  points(1:46, rca_zonal[rca_zonal$RCAID==13, 1:46], pch=16, col="lightblue")}
```
Write out rca_zonal
```{r}
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.sp[-c(388,368,387,407,408,348,427,367,329,428,385), ])
```
...
```{r}
  data.sp <- data.sp[-c(388,368,387,407,408,348,427,367,329,428,385), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e + d, data=data.fall[-c(1,20,39,58,2,96,21), ])
```
recheck for outliers
```{r}
  outlierTest(mod)
...
if outliers need to be removed (otherwise skip)
```{r}
  data.fall <- data.fall[-c(1,20,39,58,2,96,21), ]
  y <- data.fall$y
  x <- data.fall$x
  z <- data.fall$z
  e <- data.fall$e
...
```{r}
  ...
  points(newnamesnum, LogPred.out[13, 1:46], pch=16, col="blue")
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Lolo_2012_8D_Mx.mpeg')
```
