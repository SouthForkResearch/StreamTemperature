---
title: "Upper Grande Ronde Max 2013"
...
rerun w/o outliers
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(1484, 1296, 1320, 1547), ])
```
...
```{r}
  data.sp <- data.sp[-c(1484, 1296, 1320, 1547), ]
  y <- data.sp$y
  x <- data.sp$x
  z <- data.sp$z
  e <- data.sp$e
...
```{r}
  mod <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(40), ])
```
recheck assumptions
```{r}
  gvmodel <- gvlma(mod)
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png USal_2013_8D_Mx.mpeg')
```
