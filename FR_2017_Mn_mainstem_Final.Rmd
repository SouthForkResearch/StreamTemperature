---
title: "Feather River 2017 Mean Mainstem"
...
```{r}
  ...
  metricPath <- "D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/Mainstem/2017/metrics"
  ...
  modelPath <- paste0(mainPath, midBasin, "/","Mainstem", "/", yearPath, "/", longVar, "/")
...
```{r, message=FALSE}
  ...
  LST.in <- read.csv("D:/OneDrive/work/research/CHaMP/GIS/coverages/Feather_River/FR_17_LST.csv")
  PointID <- LST.in[,1]
  LST.in <- LST.in[,2:47]
...
```{r}
  ...
  write.dbf(CLST.out, file = paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/Mainstem/2017/LST17_FR_interp.dbf"))
  
  
```
This part calculates the zonal mean of the daily LST for each RCA
```{r}
out_Preds <- read.dbf("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/Mainstem/2017/LST17_FR_interp.dbf")
...
```{r}
  ...
  plot(1:46, rca_zonal[100,1:46])
    write.dbf(rca_zonal, file = paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/Mainstem/2017/LST17_FR_RCA.dbf"))
  
```
Discharge data formatting
```{r}

  Dv.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_Data/Feather/Mainstem/2017/FR_dv_data_2017main.csv", stringsAsFactors = FALSE)
...
write.csv(Dv.8Day.out, paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/Mainstem/2017/FR_2017_Dv_8Day", ".csv"), row.names = FALSE)
...
```{r}
  Log.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/Mainstem/2017/FR_logger_8D_2017_mainstem.csv", stringsAsFactors = FALSE)
 

  ID.in <- read.csv("D:/OneDrive/work/research/CHaMP/GIS/coverages/Feather_River/FR_sites_rca_2017_mainstem.csv", stringsAsFactors = FALSE) 
  
  LST.in <- read.dbf(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/Mainstem/2017/LST17_FR_RCA.dbf"))
  colnames(LST.in)<-gsub("X", "", colnames(LST.in))
  newnamesnum <- as.numeric(colnames(LST.in)[1:46]);
  Dv.in <- read.csv("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/Mainstem/2017/FR_2017_Dv_8Day.csv", stringsAsFactors = FALSE)
  ...
  write.csv(x=NoNA.xyz, file= paste0(basin, "_", yearPath, "_8Day_mainstem", var, "_model_data.csv"), row.names = FALSE)
  NoNA.xyz <- read.csv(paste0(basin, "_", yearPath, "_8Day_mainstem", var, "_model_data.csv"), stringsAsFactors = FALSE)
...
```{r}
  ...
  write.table (x=pred.out,append=T,row.names=F,file=paste0("jk_pred_v_y_mainstem", var, "_", basin, "_", yearPath, "_full_year.csv"),sep = ",", col.names=F)  
...
```{r}
 ...
 pred.y <- read.csv(paste0("jk_pred_v_y_mainstem", var, "_", basin, "_", yearPath, "_full_year.csv"), stringsAsFactors = FALSE, header = FALSE)
...
```{r}
   ...
   for (i in 1:length(rcas))  
      {
        ...
        j <- 1
        for (l in 1:midrow)
        {x[l] <- x[l] * coeffs.in$bLST[1] + j * coeffs.in$bJul[1] +  x[47] * coeffs.in$bElev[1] + Dv[l] * coeffs.in$bD[1] + x[l]^2 * coeffs.in$bLST2[1] + coeffs.in$Int[1]
        j <- j + 8}
        k <- day
        for (l in maxrow:46)     
        {x[l] <- x[l] * coeffs.in$bLST[2] + k * coeffs.in$bJul[2] +  x[47] * coeffs.in$bElev[2] + Dv[l] * coeffs.in$bD[2] + x[l]^2 * coeffs.in$bLST2[2] + coeffs.in$Int[2]
        k <- k + 8}
        if (maxrow > 3)
        {
          x[(midrow)] <- NA
  ...
  write.dbf(LogPred.out, file = paste0("predt", yearPath, "_mainstem", basin, "_8D_", var, ".dbf"))
```
Plot orig data
```{r}
...
```{r}
  ...
  Log.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/Feather/Mainstem/2017/FR_logger_8D_2017_mainstem.csv"), stringsAsFactors = FALSE)
  
  ID.in <- read.csv(paste0("D:/OneDrive/work/research/CHaMP/GIS/coverages/Feather_River/FR_sites_rca_2017_mainstem.csv"), stringsAsFactors = FALSE) 
  
  
  Pred.in <- read.dbf(paste0("predt", yearPath, "_mainstem", basin, "_8D_", var, ".dbf"))
...
write.dbf(Error.pts.out, file = paste0("Error", yearPath, "_mainstem", basin, "_8D_", var, ".dbf")) 
write.csv(Error.pts.out, file = paste0("Error", yearPath, "_mainstem", basin, "_8D_", var, ".csv"), row.names = FALSE)
```
...
```{r}
  ...
  ptsname <- paste0("TempSites_2017_mainstem")
...
setwd(paste0(modelPath, "/graphics/"))
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png FR_2017_8D_Mn_mainstem.mpeg')
```
