...
####################################################################
  ...
  weights <- read.csv(paste0(basin, "_rca_area_wgts.csv"), stringsAsFactors=FALSE)
  weights[weights$area_wgt<0.01, "area_wgts"] = 0.01
  ...
  for(i in rcas)	
    {
      pixels <- weights[weights$RCAID == i, "GRIDCODE_1"]
...
#################################################################
  ...
  plot(NoNA.xyz$x, NoNA.xyz$y, main=paste0(basin, " ", yearPath, " full year"))
  ...
  plot(pred.out[,1], pred.out[,2], main = paste0("8-day ", var, " Full year"))
 ...
 # no Julday output 
  ...
  library(pls)
  mod2 <- plsr(y ~ x + I(x^2) + z, validation = "LOO")
  p2 <- R2(mod2)
  detach("package:pls", unload=TRUE)
  ...
  points(namesnum, LogPred.out[100, 1:46], pch=16, col="blue")
...
##########################
#This parts formats the error by day/site info for spring/fall models
###########################
...
##########################
#This parts formats the error by day/site info for full year models
###########################
  ...
  e <- NoNA.xyz$e
  mod <- lm(y ~ x + I(x^2))
  pred.new <- predict(mod, data = NoNA.xyz)
  pred.new[pred.new < -0.5] = -0.5
  NoNA.xyz$pred <- unlist(pred.new)
...
# Edited 5 may 2016 to add animation output
 ...
 ################################################
  ...
  error_pts@data <- error_pts@data[,-1]
  ...
  names.out <- colnames(network@data[1:46])
  namesnum <- as.numeric(gsub(paste0("T", var, "_", yrPath, "_"), "", colnames(network@data[1:46])))
  means <- colMeans(network@data[1:46])
  SDs <- colStdevs(network@data[1:46])
  yplus <- means + SDs
  yminus <- means - SDs
  ...
  for (i in 1:46)
    {
      ...
      filename <- paste0(modelPath, "/graphics/", namer[i], ".png", sep="")
      ...
      cexEr <-ifelse(abs(error_pts@data[,i]) == 0, 0,
                  ifelse(abs(error_pts@data[,i])>0, 0.5,
                      ifelse(abs(error_pts@data[,i])>0, 0.75,
      ...
      tmp2 <- subplot(
        plot(namesnum[1:(i)], means[1:(i)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,20), cex.main=.8, main="Basin mean", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Pahs_2013_8D_mn.mpeg')
