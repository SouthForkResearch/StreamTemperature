...
setwd(shpPath)
  ...
  names.out <- colnames(network@data[4:48])
  namesnum <- as.numeric(gsub("Tmin_", "", colnames(network@data[4:48])))
  means <- colMeans(network@data[4:48], na.rm=TRUE)
  SDs <- colStdevs(network@data[4:48])
  yplus <- means + SDs
  yminus <- means - SDs
...
system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Tuc_12_13_min.mpeg')
...
# ##############################################
  ...
  shpPath <- paste0(modelPath, "graphics2/")
    ...
    for (i in 28:32)
      {
        ...
        legend(x=grconvertX(c(0.05, 0.15), from='npc'), 
               y=grconvertY(c(0.45, 0.65), from='npc'),, fill = attr(fix3.colors, "palette"), title="8-day Min (°C)", legend = c("-1-0","0-1","1-2","2-3","3-4","4-6","6-8","8-10","10-12","12+"), bty = "n", cex=.5, inset=c(.1,0), text.col="black");
        legend(x=grconvertX(c(0.89, 0.94), from='npc'), 
               y=grconvertY(c(0.19, 0.29), from='npc'),, title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.5), col = "gray70", cex=.5, text.col="black");
        
        
        title(paste0(longBasin, " 8-day min ", yearPath1, "-", yearPath2, " (°C)"), sub = paste0("Julian Day ", namey), line=-0.9, adj=.80, col.main="black", col.sub="black", outer=FALSE, cex.main=0.5, cex.sub=0.5)
        tmp2 <- subplot(
          plot(sequ[1:(i-3)], means[1:(i-3)], col=fix4.colors, pch=16, bty="n", xlim=c(1,44), ylim=c(-1,18), cex.main=.8, main="Basin mean", adj=0, xaxt='n', xlab='',ylab='', col.lab="black", cex.axis=0.5, cex.lab = 0.25, col.axis="black", col.main = "black", bg="white"), 
          x=grconvertX(c(0.1,0.45), from='npc'), 
          y=grconvertY(c(0.05, 0.20), from='npc'),
          size=c(1,1.5), vadj=0.5, hadj=0.5, 
          pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
