...
# The initial input is an LST table (columns = days, rows = RCAs), stream temperature logger data, and
# an RCAID/site crosswalk 
...
NoNA.xyz <- read.csv(file=paste0(basin, "_", yearPath, "_8Day_model_data.csv"), stringsAsFactors = FALSE)
...
###########################################
  ...
  write.table (x=data.sp,append=F,row.names=F,file=paste0("jk_pred_v_y_Mn_", basin, "_", yearPath, "_sp_fall.csv"),sep = ",", col.names=F)  
...
###########################################
  ...
  write.table (x=data.fall,append=T,row.names=F,file=paste0("jk_pred_v_y_Mn_", basin, "_", yearPath, "_sp_fall.csv"),sep = ",", col.names=F)  
...
########################################################################################################
# This part applies the model coefficients to the LST to generate temp estimates for the basin
########################################################################################################
...
###########################
  ...
  Error.pts.out[,2:47] <- as.numeric(Error.pts.out[,2:47])
  Error.pts.out[,2:47] <- round(as.numeric(Error.pts.out[,2:47], digits=3))
