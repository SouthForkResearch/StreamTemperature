...
library(classInt)
  ...
  modelPath <- paste0(mainPath, "USal/", basin, "/", yearPath)
...
setwd(paste0(mainPath, basin, "/", basin))
...
#################################################################
 ...
 # no Julday output 
  ...
  setwd(paste0(mainPath, "USal/", basin, "/", yearPath))
...
#################### graphics ############################
      ...
      cexEr <-ifelse(abs(error_pts@data[,i]) == 0, 0,
                     ifelse(abs(error_pts@data[,i])>0 & error_pts@data[,i-1]<1, 0.5,
                            ifelse(abs(error_pts@data[,i])>1 & error_pts@data[,i-1]<2, 0.75,
                                   ifelse(abs(error_pts@data[,i])>2 & error_pts@data[,i-1]<3, 1.0,
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Pahs_2011_8D_mx.mpeg')
