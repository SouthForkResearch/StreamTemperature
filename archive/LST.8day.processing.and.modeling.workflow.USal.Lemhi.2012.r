...
# Edited 5 may 2016 to add animation output
 ...
 ################################################
  ...
  namesnum <- as.numeric(gsub("TMx_12_", "", colnames(network@data[3:48])))
  ...
  for (i in 3:48)
    {
      ...
      namey <- gsub("TMx_12_", "", colnames(network@data)[i])
      ...
      title("Lemhi 8-day max 2012 (°C)", sub = paste0("Julian Day ", namey), line=-0.9, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
      tmp2 <- subplot(
  ...
  system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Lemhi_2012_8D_mx.mpeg')
