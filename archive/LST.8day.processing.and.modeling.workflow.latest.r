---
  title: "Mid Salmon/Panther Creek Mean 2012"
...
Stage setting:
  ```{r Stage setting, message=FALSE, warning=FALSE}
library(timeSeries)
library(lattice)
library(foreign)
library(doBy)
...
yrPath <- "12"
yearPath <- "2012"
dataPath <- "D:/OneDrive/work/research/CHaMP/GIS/LST/LST_s12_"
...
Log.in <- read.csv(paste0(modelPath, longBasin, "_CBS_NorWeST_", yearPath, "_logger_data.csv"), stringsAsFactors = FALSE)
...
```{r}
mod2 <- lm(y ~ x + I(x^2) + e, data=NoNA.xyz[-c(160,182,237,248,251,293), ])
```
recheck assumptions
```{r}
gvmodel <- gvlma(mod2)
...
```{r}
NoNA.xyz <- NoNA.xyz[-c(160,182,237,248,251,293), ]
y <- NoNA.xyz$y
x <- NoNA.xyz$x
z <- NoNA.xyz$z
e <- NoNA.xyz$e
...
rerun w/o outliers
```{r}
mod2 <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(5,224,237), ])
```
...
if outliers need to be removed (otherwise skip)
```{r}
data.sp <- data.sp[-c(5,103,141,221,259), ]
y <- data.sp$y
x <- data.sp$x
z <- data.sp$z
e <- data.sp$e
...
rerun w/o outliers (if necessary)
```{r}
mod2 <- lm(y ~ x + I(x^2) + z + e, data=data.sp[-c(6,9,17,79), ])
```
recheck assumptions
```{r}
gvmodel <- gvlma(mod2)
...
if outliers need to be removed (otherwise skip)
```{r}
data.fall <- data.fall[-c(6,9,17,79), ]
y <- data.fall$y
x <- data.fall$x
z <- data.fall$z
e <- data.fall$e
...
error_pts@data$SiteID <- as.character(error_pts@data$SiteID)
...
system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Panther_2012_8D_Mn.mpeg')
```
